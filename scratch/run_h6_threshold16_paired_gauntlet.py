        ...
        for p, act in ((0, act0), (1, act1)):
            ...
            for ord_item in mkt:
                if len(ord_item) >= 3 and ord_item[0] == "SELL":
                    ...
                    rev = qty * price * 0.95
                    if item == "STRAWBERRY": straw_rev[p] += rev
                    else: other_rev[p] += rev
...
print("=" * 125)
print("PHASE-11 SCIENTIFIC EVALUATION: RC2 CONTROL vs H6-THRESHOLD16 (20-MATCH PAIRED GAUNTLET)")
print("=" * 125)
...
# --- SUMMARY STATISTICS ---
print("\n" + "=" * 125)
print(f"{'METRIC':<30} | {'RC2 CONTROL':>15} | {'H6-THRESHOLD16':>15} | {'DELTA (H6 - RC2)':>18}")
print("-" * 125)
...
with open("reports/H6_THRESHOLD16_PAIRED_AUDIT.json", "w") as f:
    json.dump({
        "rc2_scores": rc2_s,
        "h6_scores": h6_s,
...
print("\nComplete audit saved to reports/H6_THRESHOLD16_PAIRED_AUDIT.json")
