...
print("=" * 105)
print("     OFFICIAL 5-SEED GAUNTLET: RC2 CONTROL vs RC3-E TERMINAL REALIZATION     ")
print("=" * 105)

rc2_scores, rc2_plants, rc2_total, rc2_avg = run_agent_eval("submission_rc2_terminal_horizon", "RC2 Control (Frozen Baseline)")
rc3e_scores, rc3e_plants, rc3e_total, rc3e_avg = run_agent_eval("submission_rc3e_laboratory", "RC3-E Terminal Realization")

print("\n" + "=" * 105)
print("     COMPARATIVE HEAD-TO-HEAD LEADERBOARD     ")
print("=" * 105)
print(f"  {'Opponent / Seed':<26} | {'RC2 Control':>12} | {'RC3-E Lab':>12} | {'Net Lift':>12}")
print("-" * 105)
for _, _, opp_name in replays:
    s2 = rc2_scores[opp_name]
    s3e = rc3e_scores[opp_name]
    lift = s3e - s2
    print(f"  {opp_name:<26} | ${s2:>10,.0f} | ${s3e:>10,.0f} | ${lift:>+10,.0f}")
print("-" * 105)
total_lift = rc3e_total - rc2_total
print(f"  {'TOTAL AGGREGATE':<26} | ${rc2_total:>10,.0f} | ${rc3e_total:>10,.0f} | ${total_lift:>+10,.0f}")
print(f"  {'AVERAGE PER MATCH':<26} | ${rc2_avg:>10,.0f} | ${rc3e_avg:>10,.0f} | ${rc3e_avg - rc2_avg:>+10,.0f}")
print("=" * 105)
