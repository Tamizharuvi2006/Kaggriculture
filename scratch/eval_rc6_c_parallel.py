...
def run_evaluation():
    ...
    cand_rc6c = r"D:\kaggriculture\submission_rc6_c.py"
    
    print(f"Executing 20-match paired evaluation (RC4.2 vs RC6-C) across 10 parallel workers...")
    t0 = time.time()
    
    tasks_rc42 = [(cand_rc42, r_path, seat) for r_path, seat in SELECTED_REPLAYS]
    tasks_rc6c = [(cand_rc6c, r_path, seat) for r_path, seat in SELECTED_REPLAYS]
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=10) as executor:
        for s in executor.map(_run_single_match, tasks_rc6c):
            scores_rc6c.append(s)
            
    elapsed = time.time() - t0
    print(f"All 40 matches finished in {elapsed:.2f} seconds! (10 parallel workers)\n")
    
    deltas = np.array(scores_rc6c) - np.array(scores_rc42)
    ...
    print("=" * 95)
    print(f"{'MATCH':<5} | {'REPLAY ID':<20} | {'RC4.2 SCORE':<14} | {'RC6-C SCORE':<14} | {'DELTA':<12} | {'OUTCOME'}")
    print("-" * 95)
    for i, (r_path, seat) in enumerate(SELECTED_REPLAYS):
        ...
        print(f"#{i+1:<4} | {r_name:<20} | ${s42:>11,} | ${s6c:>11,} | ${d:>+10,} | {outcome}")
    print("=" * 95)
    ...
    print(f"  RC6-C Mean:        ${np.mean(scores_rc6c):>10,.0f}  (DELTA: ${np.mean(deltas):>+10,.0f})")
    print(f"  RC4.2 Median:      ${np.median(scores_rc42):>10,.0f}")
    print(f"  RC6-C Median:      ${np.median(scores_rc6c):>10,.0f}")
    print(f"  RC4.2 Floor (Min): ${np.min(scores_rc42):>10,.0f}")
    print(f"  RC6-C Floor (Min): ${np.min(scores_rc6c):>10,.0f}  (DELTA: ${np.min(scores_rc6c)-np.min(scores_rc42):>+10,.0f})")
    print(f"  RC4.2 Ceiling:     ${np.max(scores_rc42):>10,.0f}")
    print(f"  RC6-C Ceiling:     ${np.max(scores_rc6c):>10,.0f}")
