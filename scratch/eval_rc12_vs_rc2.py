...
def run_evaluation():
    cand_rc2 = r"D:\kaggriculture\submission_rc2_rollback.py"
    cand_rc12 = r"D:\kaggriculture\submission_rc12.py"
    
    print(f"Executing 20-match paired evaluation (Live Champion RC2 vs RC12) across 10 parallel workers...")
    t0 = time.time()
    
    tasks_rc2 = [(cand_rc2, r_path, seat) for r_path, seat in SELECTED_REPLAYS]
    tasks_rc12 = [(cand_rc12, r_path, seat) for r_path, seat in SELECTED_REPLAYS]
    
    scores_rc2 = []
    scores_rc12 = []
    
    with concurrent.futures.ProcessPoolExecutor(max_workers=10) as executor:
        for s in executor.map(_run_single_match, tasks_rc2):
            scores_rc2.append(s)
            
    with concurrent.futures.ProcessPoolExecutor(max_workers=10) as executor:
        for s in executor.map(_run_single_match, tasks_rc12):
            scores_rc12.append(s)
            
    elapsed = time.time() - t0
    print(f"All 40 matches finished in {elapsed:.2f} seconds! (10 parallel workers)\n")
    
    deltas = np.array(scores_rc12) - np.array(scores_rc2)
    ...
    print("=" * 95)
    print(f"{'MATCH':<5} | {'REPLAY ID':<20} | {'RC2 (PROD)':<14} | {'RC12 SCORE':<14} | {'DELTA':<12} | {'OUTCOME'}")
    print("-" * 95)
    for i, (r_path, seat) in enumerate(SELECTED_REPLAYS):
        ...
        print(f"#{i+1:<4} | {r_name:<20} | ${s2:>11,} | ${s12:>11,} | ${d:>+10,} | {outcome}")
    print("=" * 95)
    
    print("\n" + "=" * 60)
    print("PAIRED STATISTICAL SUMMARY (20 DIVERSE MATCHES):")
    print("=" * 60)
    print(f"  RC2 (Live Prod) Mean: ${np.mean(scores_rc2):>10,.0f}")
    print(f"  RC12 Mean:            ${np.mean(scores_rc12):>10,.0f}  (DELTA: ${np.mean(deltas):>+10,.0f} / +{(np.mean(scores_rc12)/np.mean(scores_rc2)-1)*100:.1f}%)")
    print(f"  RC2 Median:           ${np.median(scores_rc2):>10,.0f}")
    print(f"  RC12 Median:          ${np.median(scores_rc12):>10,.0f}")
    print(f"  RC2 Floor (Min):      ${np.min(scores_rc2):>10,.0f}")
    print(f"  RC12 Floor (Min):     ${np.min(scores_rc12):>10,.0f}  (DELTA: ${np.min(scores_rc12)-np.min(scores_rc2):>+10,.0f} / +{(np.min(scores_rc12)/np.min(scores_rc2)-1)*100:.1f}%)")
    print(f"  RC2 Ceiling:          ${np.max(scores_rc2):>10,.0f}")
    print(f"  RC12 Ceiling:         ${np.max(scores_rc12):>10,.0f}  (DELTA: ${np.max(scores_rc12)-np.max(scores_rc2):>+10,.0f})")
    print(f"  Record vs RC2:        {wins} Wins / {losses} Losses / {ties} Ties")
