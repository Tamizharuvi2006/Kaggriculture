...
import submission_rc2_market_advisor_v01 as adv_mod
...
def run_single_game(agent_p0, agent_p1, seed, opp_tape=None, max_steps=720):
    ...
    # Track when advisor actually reorders
    reorders = {0: 0, 1: 0}
    ...
    for step in range(max_steps):
        ...
        # Check advisor activation condition
        for p, obs in ((0, obs0), (1, obs1)):
            mkt = obs.market
            prices = mkt.prices
            inv = mkt.inventory
            m_risk = (float(prices.get("MILK", 0) or 0) >= 180.0 and int(inv.get("MILK", 10000)) >= 10000)
            s_risk = (float(prices.get("STRAWBERRY", 0) or 0) >= 120.0 and int(inv.get("STRAWBERRY", 10000)) >= 10000)
            if m_risk or s_risk:
                reorders[p] += 1
    ...
    for p in (0, 1):
        ...
        res[p] = {
            "money": f.money,
            "reorder_events": reorders[p]
        }
    return res

print("=" * 115)
print("PHASE-11 SCIENTIFIC EVALUATION: RC2 CONTROL vs RC2 + MARKET ADVISOR v0.1 (PAIRED SEAT-SWAPPED)")
print("=" * 115)
...
for r_name, opp_label in replays:
    ...
    results["ADV"]["reorders"].extend([r_adv_0["reorder_events"], r_adv_1["reorder_events"]])
    
    print(f"  {opp_label:<26} | S0 RC2: ${r_rc2_0['money']:>7,.0f} | ADV: ${r_adv_0['money']:>7,.0f} | Diff: ${r_adv_0['money']-r_rc2_0['money']:>+6,.0f} (Triggers: {r_adv_0['reorder_events']})")
    print(f"  {'':<26} | S1 RC2: ${r_rc2_1['money']:>7,.0f} | ADV: ${r_adv_1['money']:>7,.0f} | Diff: ${r_adv_1['money']-r_rc2_1['money']:>+6,.0f} (Triggers: {r_adv_1['reorder_events']})")
    sys.stdout.flush()
...
for seed, label in low_seeds:
    ...
    results["ADV"]["reorders"].extend([adv_m1["reorder_events"], adv_m2["reorder_events"]])
...
print(f"Average Advisor Reorder Triggers per Match: {sum(results['ADV']['reorders'])/len(results['ADV']['reorders']):.1f} turns")
print("=" * 115)

# Save audit report
with open("reports/ADVISOR_V01_PAIRED_AUDIT.json", "w") as f:
    json.dump({
        "rc2_scores": rc2_s,
        "adv_scores": adv_s,
...
print("\nComplete audit saved to reports/ADVISOR_V01_PAIRED_AUDIT.json")
