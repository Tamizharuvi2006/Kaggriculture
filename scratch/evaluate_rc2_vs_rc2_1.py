...
print("=" * 105)
print("     OFFICIAL HEAD-TO-HEAD BENCHMARK: RC2 CONTROL vs RC2.1 TOMATO FIX     ")
print("=" * 105)

rc2_scores, rc2_plants, rc2_total, rc2_avg = run_agent_eval("submission_rc2_terminal_horizon", "RC2 Control (Frozen Baseline)")
rc21_scores, rc21_plants, rc21_total, rc21_avg = run_agent_eval("submission_rc2_1_tomato_fix", "RC2.1 Tomato Fix")

print("\n" + "=" * 105)
print("     COMPARATIVE HEAD-TO-HEAD LEADERBOARD     ")
print("=" * 105)
print(f"  {'Opponent / Seed':<26} | {'RC2 Control':>12} | {'RC2.1 Fix':>12} | {'Net Lift':>12}")
print("-" * 105)
for _, _, opp_name in replays:
    s2 = rc2_scores[opp_name]
    s21 = rc21_scores[opp_name]
    lift = s21 - s2
    print(f"  {opp_name:<26} | ${s2:>10,.0f} | ${s21:>10,.0f} | ${lift:>+10,.0f}")
print("-" * 105)
total_lift = rc21_total - rc2_total
print(f"  {'TOTAL AGGREGATE':<26} | ${rc2_total:>10,.0f} | ${rc21_total:>10,.0f} | ${total_lift:>+10,.0f}")
print(f"  {'AVERAGE PER MATCH':<26} | ${rc2_avg:>10,.0f} | ${rc21_avg:>10,.0f} | ${rc21_avg - rc2_avg:>+10,.0f}")
print("=" * 105)
