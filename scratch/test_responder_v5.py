...
def run_sim(args):
    ...
    def policy_agent(obs):
        ...
        if policy_mode == "responder_v4":
        ...
        elif policy_mode == "responder_v5":
            # 1. LAND PREEMPTION WITH STRICT RUNWAY PROTECTION ($930 for Q2, $1,150 for Q3)
            # Only preempt if opponent has already unlocked the quadrant!
            ...
            # 2. ANIMAL CAPACITY MATCHING (COWS ONLY) WITH $850 CASH BUFFER
            # Protects working capital so crops never starve
            if own_quads >= 2 and 8 <= day <= 15 and opp_animals >= 4:
                if own_cows < 8 and money >= 850.0 and own_animals < opp_animals + 1:
                    if not any(isinstance(m, (list, tuple)) and len(m) >= 2 and m[0] == "BUY_ANIMAL" for m in mkt):
                        mkt.append(["BUY_ANIMAL", "COW"])
                        
            # 3. FEED ECONOMICS CUTOFF (DAY 22+)
            # Do not spend $40 on open market feed late game when wool sells for < $40
...
def main():
    print("=========================================================================================")
    print("     PARALLEL 12-WORKER HEAD-TO-HEAD: BASELINE vs RESPONDER V4 vs RESPONDER V5          ")
    print("=========================================================================================")
    
    targets = [
        (r"D:\kaggriculture\reports\live_match_telemetry\episode-104475527-replay.json", 0), # vs RicardoLopez (-$43k)
        (r"D:\kaggriculture\reports\live_match_telemetry\episode-104424149-replay.json", 0), # vs JZ (-$38k)
        (r"D:\kaggriculture\reports\live_match_telemetry\episode-104433117-replay.json", 0), # vs ayman elamin (-$28k)
        (r"D:\kaggriculture\reports\live_match_telemetry\episode-104388418-replay.json", 0), # vs Soumi Ghosh (-$26k)
        (r"D:\kaggriculture\reports\live_match_telemetry\episode-104379472-replay.json", 0), # vs arao (-$14k)
    ...
    modes = ["baseline", "responder_v4", "responder_v5"]
    ...
    print("=" * 135)
    print(f"{'Replay Match':<28} | {'Baseline':<12} | {'Responder V4':<13} | {'Responder V5':<13} | {'V5 vs Base':<12} | {'V5 vs V4':<12} | Status")
    print("=" * 135)
    
    base_tot, v4_tot, v5_tot = 0.0, 0.0, 0.0
    for (fname, opp_name), data in sorted(results.items()):
        b_c, _ = data["baseline"]
        v4_c, _ = data["responder_v4"]
        v5_c, opp_rew = data["responder_v5"]
        base_tot += b_c
        v4_tot += v4_c
        v5_tot += v5_c
        d_base = v5_c - b_c
        d_v4 = v5_c - v4_c
        status = "WIN [W]" if v5_c > opp_rew else "LOSS"
        lbl = f"{fname[:10]} vs {opp_name[:14]}"
        print(f"{lbl:<28} | ${b_c:10,.0f} | ${v4_c:11,.0f} | ${v5_c:11,.0f} | {d_base:+10,.0f} | {d_v4:+10,.0f} | {status}")
        
    n = len(results)
    print("=" * 135)
    print(f"{'MEAN WEALTH ACROSS ALL 10':<28} | ${base_tot/n:10,.0f} | ${v4_tot/n:11,.0f} | ${v5_tot/n:11,.0f} | {v5_tot/n - base_tot/n:+10,.0f} | {v5_tot/n - v4_tot/n:+10,.0f} |")
    print("=" * 135)
