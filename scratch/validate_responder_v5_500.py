...
def run_match(args):
    ...
    def v5_agent(obs):
        ...
        farms = obs.get("farms", [{}, {}])
        own = farms[v5_seat] if len(farms) > v5_seat else {}
        opp = farms[1 - v5_seat] if len(farms) > (1 - v5_seat) else {}
        ...
        # 1. Land Preemption with strict runway buffer ($930 for Q2, $1,150 for Q3)
    ...
    v5_reward = env.state[v5_seat].reward
    base_reward = env.state[1 - v5_seat].reward
    
    win = 1 if v5_reward > base_reward else (0.5 if v5_reward == base_reward else 0)
    delta = v5_reward - base_reward
    
    return seed, v5_seat, v5_reward, base_reward, win, delta

def main():
    print("=========================================================================================")
    print("     500-MATCH COMPREHENSIVE TOURNAMENT: RESPONDER V5 vs LIVE CLEAN CANDIDATE            ")
    print("=========================================================================================")
    
    seeds = [4000000 + i * 23 for i in range(250)]
    tasks = []
    for s in seeds:
        tasks.append((s, 0))
        tasks.append((s, 1))
    ...
    v5_scores = [r[2] for r in results]
    ...
    mean_v5 = np.mean(v5_scores)
    mean_base = np.mean(base_scores)
    mean_delta = np.mean(deltas)
    
    tail_v5_5th = np.percentile(v5_scores, 5)
    ...
    print(f"Mean Wealth (V5)       : ${mean_v5:,.0f}")
    print(f"Mean Wealth (Live Base): ${mean_base:,.0f}")
    print(f"Net Delta per Game     : {mean_delta:+,.0f}")
    print(f"Tail Floor (5th %ile)  : V5 ${tail_v5_5th:,.0f} vs Baseline ${tail_base_5th:,.0f} (Delta: {tail_v5_5th - tail_base_5th:+,.0f})")
