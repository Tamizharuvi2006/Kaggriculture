...
def run_sim(args):
    ...
    def policy_agent(obs):
        ...
        if policy_mode == "responder_v3":
            # Responder v3:
            # 1. Do NOT rush Land #2 on Day 5 (let baseline accumulate $1,400+ naturally for Day 7).
            # 2. On Day 8-15 (after Land #2 is open): if opponent is scaling livestock (opp_animals >= 4),
            #    and we have safe surplus capital (money >= $850), reinvest in Cows (up to 8 cows).
            if own_quads >= 2 and 8 <= day <= 16 and opp_animals >= 4:
                if own_cows < 8 and money >= 850.0 and own_animals < opp_animals + 2:
                    if not any(isinstance(m, (list, tuple)) and len(m) >= 2 and m[0] == "BUY_ANIMAL" for m in mkt):
                        mkt.append(["BUY_ANIMAL", "COW"])
                        
            # 3. On Day 10-12: If money >= 1200 and own_quads == 2 and opp_animals >= 6:
            #    Secure Quadrant 3 early without risking cash flow
            if own_quads == 2 and 10 <= day <= 12 and money >= 1200.0 and opp_animals >= 6:
...
def main():
    print("=========================================================================================")
    print("     PARALLEL 12-WORKER HEAD-TO-HEAD: BASELINE vs RESPONDER V3 (SURPLUS REINVESTMENT)    ")
    print("=========================================================================================")
    ...
    print("=" * 115)
    print(f"{'Replay Match':<30} | {'Baseline':<12} | {'Responder V3':<14} | {'Net Delta ($)':<14} | {'Opponent ($)':<12} | Status")
    print("=" * 115)
    
    base_tot, v3_tot = 0.0, 0.0
    for (fname, opp_name), data in sorted(results.items()):
        b_c, _ = data["baseline"]
        v3_c, opp_rew = data["responder_v3"]
        base_tot += b_c
        v3_tot += v3_c
        delta = v3_c - b_c
        status = "WIN [W]" if v3_c > opp_rew else "LOSS"
        lbl = f"{fname[:12]} vs {opp_name[:12]}"
        print(f"{lbl:<30} | ${b_c:10,.0f} | ${v3_c:12,.0f} | {delta:+12,.0f} | ${opp_rew:10,.0f} | {status}")
        
    n = len(results)
    print("=" * 115)
    print(f"{'MEAN WEALTH ACROSS ALL 10':<30} | ${base_tot/n:10,.0f} | ${v3_tot/n:12,.0f} | {v3_tot/n - base_tot/n:+12,.0f} |")
    print("=" * 115)
