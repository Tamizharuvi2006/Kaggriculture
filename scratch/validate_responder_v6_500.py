...
def run_match(args):
    ...
    def v6_agent(obs):
        ...
        farms = obs.get("farms", [{}, {}])
        own = farms[v6_seat] if len(farms) > v6_seat else {}
        opp = farms[1 - v6_seat] if len(farms) > (1 - v6_seat) else {}
    ...
    v6_reward = env.state[v6_seat].reward
    base_reward = env.state[1 - v6_seat].reward
    
    win = 1 if v6_reward > base_reward else (0.5 if v6_reward == base_reward else 0)
    delta = v6_reward - base_reward
    
    return seed, v6_seat, v6_reward, base_reward, win, delta

def main():
    print("=========================================================================================")
    print("     500-MATCH COMPREHENSIVE TOURNAMENT: RESPONDER V6 vs LIVE CLEAN CANDIDATE            ")
    print("=========================================================================================")
    
    seeds = [5000000 + i * 29 for i in range(250)]
    tasks = []
    for s in seeds:
        tasks.append((s, 0))
        tasks.append((s, 1))
    ...
    v6_scores = [r[2] for r in results]
    ...
    mean_v6 = np.mean(v6_scores)
    mean_base = np.mean(base_scores)
    mean_delta = np.mean(deltas)
    
    tail_v6_5th = np.percentile(v6_scores, 5)
    ...
    print(f"Mean Wealth (V6)       : ${mean_v6:,.0f}")
    print(f"Mean Wealth (Live Base): ${mean_base:,.0f}")
    print(f"Net Delta per Game     : {mean_delta:+,.0f}")
    print(f"Tail Floor (5th %ile)  : V6 ${tail_v6_5th:,.0f} vs Baseline ${tail_base_5th:,.0f} (Delta: {tail_v6_5th - tail_base_5th:+,.0f})")
