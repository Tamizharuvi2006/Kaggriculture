...
def check_match(args):
    ...
    for s in range(len(steps)-1):
        ...
        for o in act.get("market", []):
            if o and "BUY_ANIMAL" in o[0]:
                animal_buys.append((day, o[1], o[2]))
        if hero_seat == 0:
    ...
    return idx, final_score, animal_buys

if __name__ == "__main__":
    tasks = [(i+1, m) for i, m in enumerate(SELECTED_REPLAYS)]
    with concurrent.futures.ProcessPoolExecutor(max_workers=10) as ex:
        res = list(ex.map(check_match, tasks))
    for idx, score, buys in res:
        print(f"Match #{idx:02d}: Score=${score:>6,d} | Animal Buys: {buys}")
