...
def run_evaluation():
    ...
    print(f"Executing 20-match paired evaluation (RC4.2 vs RC12) across 10 parallel workers...")
    t0 = time.time()
    ...
    deltas = np.array(scores_rc12) - np.array(scores_rc42)
    ...
    print("=" * 95)
    print(f"{'MATCH':<5} | {'REPLAY ID':<20} | {'RC4.2 SCORE':<14} | {'RC12 SCORE':<14} | {'DELTA':<12} | {'OUTCOME'}")
    print("-" * 95)
    for i, (r_path, seat) in enumerate(SELECTED_REPLAYS):
        ...
        print(f"#{i+1:<4} | {r_name:<20} | ${s42:>11,} | ${s12:>11,} | ${d:>+10,} | {outcome}")
    print("=" * 95)
    ...
    print(f"  RC12 Mean:         ${np.mean(scores_rc12):>10,.0f}  (DELTA: ${np.mean(deltas):>+10,.0f})")
    ...
    print(f"  RC12 Floor (Min):  ${np.min(scores_rc12):>10,.0f}  (DELTA: ${np.min(scores_rc12)-np.min(scores_rc42):>+10,.0f})")
    print(f"  RC4.2 Ceiling:     ${np.max(scores_rc42):>10,.0f}")
    print(f"  RC12 Ceiling:      ${np.max(scores_rc12):>10,.0f}")
