...
def run_evaluation():
    ...
    cand_rc6d1 = r"D:\kaggriculture\submission_rc6_d1.py"
    
    print(f"Executing 20-match paired evaluation (Live Champion RC2 vs RC6-D.1) across 10 parallel workers...")
    t0 = time.time()
    
    tasks_rc2 = [(cand_rc2, r_path, seat) for r_path, seat in SELECTED_REPLAYS]
    tasks_rc6d1 = [(cand_rc6d1, r_path, seat) for r_path, seat in SELECTED_REPLAYS]
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=10) as executor:
        for s in executor.map(_run_single_match, tasks_rc6d1):
            scores_rc6d1.append(s)
            
    elapsed = time.time() - t0
    print(f"All 40 matches finished in {elapsed:.2f} seconds! (10 parallel workers)\n")
    
    deltas = np.array(scores_rc6d1) - np.array(scores_rc2)
    ...
    print("=" * 95)
    print(f"{'MATCH':<5} | {'REPLAY ID':<20} | {'RC2 (PROD)':<14} | {'RC6-D.1':<14} | {'DELTA':<12} | {'OUTCOME'}")
    print("-" * 95)
    for i, (r_path, seat) in enumerate(SELECTED_REPLAYS):
        ...
        print(f"#{i+1:<4} | {r_name:<20} | ${s2:>11,} | ${sd1:>11,} | ${d:>+10,} | {outcome}")
    print("=" * 95)
    ...
    print(f"  RC6-D.1 Mean:         ${np.mean(scores_rc6d1):>10,.0f}  (DELTA: ${np.mean(deltas):>+10,.0f} / +{(np.mean(scores_rc6d1)/np.mean(scores_rc2)-1)*100:.1f}%)")
    print(f"  RC2 Median:           ${np.median(scores_rc2):>10,.0f}")
    print(f"  RC6-D.1 Median:       ${np.median(scores_rc6d1):>10,.0f}")
    print(f"  RC2 Floor (Min):      ${np.min(scores_rc2):>10,.0f}")
    print(f"  RC6-D.1 Floor (Min):  ${np.min(scores_rc6d1):>10,.0f}  (DELTA: ${np.min(scores_rc6d1)-np.min(scores_rc2):>+10,.0f})")
    print(f"  RC2 Ceiling:          ${np.max(scores_rc2):>10,.0f}")
    print(f"  RC6-D.1 Ceiling:      ${np.max(scores_rc6d1):>10,.0f}  (DELTA: ${np.max(scores_rc6d1)-np.max(scores_rc2):>+10,.0f})")
