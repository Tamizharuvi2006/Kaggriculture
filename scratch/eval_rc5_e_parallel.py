...
def run_evaluation():
    ...
    cand_rc5e = r"D:\kaggriculture\submission_rc5_e.py"
    
    print(f"Executing 20-match paired evaluation (RC4.2 vs RC5-E) across 10 parallel workers...")
    t0 = time.time()
    
    tasks_rc42 = [(cand_rc42, r_path, seat) for r_path, seat in SELECTED_REPLAYS]
    tasks_rc5e = [(cand_rc5e, r_path, seat) for r_path, seat in SELECTED_REPLAYS]
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=10) as executor:
        for s in executor.map(_run_single_match, tasks_rc5e):
            scores_rc5e.append(s)
            
    elapsed = time.time() - t0
    print(f"All 40 matches finished in {elapsed:.2f} seconds! (10 parallel workers)\n")
    
    deltas = np.array(scores_rc5e) - np.array(scores_rc42)
    ...
    print("=" * 95)
    print(f"{'MATCH':<5} | {'REPLAY ID':<20} | {'RC4.2 SCORE':<14} | {'RC5-E SCORE':<14} | {'DELTA':<12} | {'OUTCOME'}")
    print("-" * 95)
    for i, (r_path, seat) in enumerate(SELECTED_REPLAYS):
        ...
        print(f"#{i+1:<4} | {r_name:<20} | ${s42:>11,} | ${se:>11,} | ${d:>+10,} | {outcome}")
    print("=" * 95)
    ...
    print(f"  RC5-E Mean:        ${np.mean(scores_rc5e):>10,.0f}  (DELTA: ${np.mean(deltas):>+10,.0f})")
    print(f"  RC4.2 Median:      ${np.median(scores_rc42):>10,.0f}")
    print(f"  RC5-E Median:      ${np.median(scores_rc5e):>10,.0f}")
    print(f"  RC4.2 Floor (Min): ${np.min(scores_rc42):>10,.0f}")
    print(f"  RC5-E Floor (Min): ${np.min(scores_rc5e):>10,.0f}  (DELTA: ${np.min(scores_rc5e)-np.min(scores_rc42):>+10,.0f})")
    print(f"  RC4.2 Ceiling:     ${np.max(scores_rc42):>10,.0f}")
    print(f"  RC5-E Ceiling:     ${np.max(scores_rc5e):>10,.0f}")
