"""Build Monolithic 100% Standalone APEX 3.1 Kaggle Submission Artifact.

Integrates:
1. Frozen L+ 4.1 Master Strategy Core (kaitofukami-v18.py)
2. WorldState & CashFlowState dynamic parsing
3. Environment-Parity Calibrated MCV Evaluator (EmpiricalMarginalEvaluator31 - 24-step market clearance)
...
Outputs: generalization_pipeline/submission_candidate_apex31.py
"""
...
OUT_PATH = os.path.join(BASE_DIR, "generalization_pipeline", "submission_candidate_apex31.py")

with open(V18_PATH, "r", encoding="utf-8") as f:
    v18_code = f.read()

# Rename base agent entrypoint in v18 code so APEX 3.1 wraps it as agent(obs, configuration)
v18_code = v18_code.replace("def agent(obs):", "def _base_agent(obs, configuration=None):")

APEX_31_EMBEDDED_CODE = '''

# ====================================================================================================
# 🏛️ APEX 3.1 STANDALONE MONOLITHIC RUNTIME ENGINE (ENVIRONMENT PARITY 24-STEP MCV)
# ====================================================================================================
    ...
    def calculate_marginal_value(candidate, expert_action, state):
        ...
        if unlocks_hire:
            ...
            if current_money < 300.0:
                capital_multiplier = 0.002 # 24-step market clearing distress penalty
...
# Global APEX 3.1 State Instance
...
def agent(obs, configuration=None):
    ...
    for cand in candidates:
        ...
        if safe:
            mcv_score, _ = EmpiricalMarginalEvaluator31.calculate_marginal_value(cand, base_action, wstate)
            if mcv_score >= 1.0:
...
# Merge Base Engine + APEX 3.1 Engine
final_code = v18_code + "\n" + APEX_31_EMBEDDED_CODE

with open(OUT_PATH, "w", encoding="utf-8") as f:
    f.write(final_code)

print(f"Successfully generated standalone APEX 3.1 Monolithic Artifact at:\n  {OUT_PATH}")
