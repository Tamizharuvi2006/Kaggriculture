"""EXP154 Fast Runner: Day-30 Labor EV Mining across Archetypes."""
...
def main():
    print("=" * 145)
    print("EXP154: DAY-30 LABOR VALUE SWEEP & DECISION MODELING (N=0, 2, 4, 10 ACROSS 10 ARCHETYPES)")
    print("=" * 145)
    ...
    for idx, chunk in enumerate(chunks):
        ...
        cmd = [sys.executable, os.path.join(BASE_DIR, "experiments", "exp154_fast_worker.py"), chunk_str, worker_id]
    ...
    for idx in range(len(chunks)):
        worker_id = f"worker_{idx}"
        part_file = os.path.join(REPORTS_DIR, f"exp154_fast_part_{worker_id}.json")
    ...
    print("\n" + "=" * 145)
    print("1. DISTRIBUTION OF OPTIMAL DAY-30 HIRES (N*) ACROSS MATCH STATES:")
    print("=" * 145)
    n_counts = {n: sum(1 for d in all_data if d["best_n"] == n) for n in [0, 2, 4, 10]}
