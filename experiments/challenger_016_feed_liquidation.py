"""Challenger 016: Late-Game Feed Conservation & Surplus Wheat Liquidation.
Tests whether stopping wheat purchases after Day 28 (when cows can no longer complete a full milk cycle)
and liquidating surplus wheat yields positive terminal delta over Variant D.1.
"""
...
class Challenger016Agent:
    """Challenger 016: Variant D.1 with Day 28+ Wheat Feed Conservation and Wheat Surplus Liquidation."""
    def __init__(self):
    ...
    def act(self, raw_obs, raw_config=None):
        try:
            ...
            # Challenger 016 Modification:
            # 1. On Days 28+ (Steps 672+), cancel any wheat purchases (cows cannot finish another 2-day milk cycle)
            if day >= 28:
                market_orders = [m for m in market_orders if not (len(m) >= 2 and m[0] == "BUY_PRODUCT" and m[1] == "WHEAT")]
                
                # 2. Sell surplus wheat in shed (keeping 0 since game ends)
                wheat_qty = farm.shed.get("WHEAT", 0)
                if wheat_qty > 0 and not any(len(m) >= 2 and m[0] == "SELL" and m[1] == "WHEAT" for m in market_orders):
                    if len(market_orders) < 10:
                        market_orders.append(["SELL", "WHEAT", wheat_qty])

            # Dynamic Selling (Standard Variant D.1)
...
def run_challenger_016_tournament():
    print("=" * 95)
    print("CHALLENGER 016: DAY 28+ FEED CONSERVATION & WHEAT CLEARANCE vs VARIANT D.1 CONTROL")
    print("=" * 95)
    ...
    print("\n" + "=" * 95)
    print("CHALLENGER 016 TOURNAMENT RESULTS (16 SEEDS x 2 SEATS = 32 MATCHES)")
    print("=" * 95)
    print(f"Direct Matchup: Challenger 016 (Day 28 Wheat Clearance) vs Variant D.1 Control")
    ...
    print(f"  - Challenger 016 Win Rate      : {report_vs_v18_chal['overall_win_rate']:.1%} | Margin: ${report_vs_v18_chal['mean_paired_delta']:+,.2f}")

    print("\n" + "=" * 95)
    if report_h2h['overall_win_rate'] >= 0.50 and report_h2h['mean_paired_delta'] > 20.0:
        print(">>> VERDICT: CHALLENGER 016 BEATS VARIANT D.1! (PROMOTE TO NEW CHAMPION)")
    else:
        print(">>> VERDICT: CHALLENGER 016 FAILS TO BEAT VARIANT D.1! (KEEP VARIANT D.1)")
    print("=" * 95)

if __name__ == "__main__":
    run_challenger_016_tournament()
