"""Build Monolithic 100% Standalone APEX 3.2 Kaggle Submission Artifact.

Integrates:
1. Frozen L+ 4.1 Master Strategy Core (kaitofukami-v18.py)
2. WorldState dynamic parsing
3. ActionPlanner: Artificial Wheat Fallback Injection REMOVED. Pure candidate generation.
4. Environment-Parity Evaluator (24-step Town Center clearance lock awareness)
5. ActionSafetyGate (Zero-Capital-Cost Invariant + Market Occupancy Guard)
...
Outputs: generalization_pipeline/submission_candidate_apex32.py
"""
...
OUT_PATH = os.path.join(BASE_DIR, "generalization_pipeline", "submission_candidate_apex32.py")

with open(V18_PATH, "r", encoding="utf-8") as f:
    v18_code = f.read()

# Rename base agent entrypoint in v18 code so APEX 3.2 wraps it as agent(obs, configuration)
v18_code = v18_code.replace("def agent(obs):", "def _base_agent(obs, configuration=None):")

APEX_32_EMBEDDED_CODE = '''

# ====================================================================================================
# 🏛️ APEX 3.2 STANDALONE MONOLITHIC RUNTIME ENGINE (BUG-FIXED CANDIDATE GENERATION)
# ====================================================================================================
...
class EmpiricalMarginalEvaluator32:
    @staticmethod
...
class ActionPlanner:
    ...
    def generate_market_candidates(state, expert_act):
        ...
        # Pure candidate generation: NO artificial fallback candidates injected!
        return candidates

class DivergenceController:
    def __init__(self, max_deviations_per_episode=1):
    ...
    def select_controlled_deviation(self, approved_candidates, state):
        if self.executed_count >= self.max_deviations_per_episode or not approved_candidates:
            return None
...
# Global APEX 3.2 State Instance
_APEX_CONTROLLER = DivergenceController(max_deviations_per_episode=1)
...
def agent(obs, configuration=None):
    ...
    for cand in candidates:
        ...
        if safe:
            mcv_score, _ = EmpiricalMarginalEvaluator32.calculate_marginal_value(cand, base_action, wstate)
            if mcv_score >= 1.0:
...
# Merge Base Engine + APEX 3.2 Engine
final_code = v18_code + "\n" + APEX_32_EMBEDDED_CODE

with open(OUT_PATH, "w", encoding="utf-8") as f:
    f.write(final_code)

print(f"Successfully generated standalone APEX 3.2 Monolithic Artifact at:\n  {OUT_PATH}")
