"""EXP130 3-Seed Proof: Sequential Endgame Defense Validation (Milk & Strawberry).

Tests:
- Arm A: Control Baseline (D.1 + Day 30 Labor Burst)
- Arm B: Arm A + Milk-Collapse Defense (Days 26-29: permit immediate milk liquidation if p_milk <= $95.0)
- Arm C: Arm A + Strawberry-Collapse Defense (Days 27-29: permit early strawberry liquidation if p_straw <= $125.0)
- Arm D: Combined Policy (Arm A + Arm B + Arm C)
"""
...
class EXP130Agent:
    ...
    def act(self, obs: dict, config=None) -> dict:
        ...
        # Check market prices and shed inventory
...
def test_single_seed(seed: int, seat: int = 0, label: str = ""):
    print(f"\n--- {label}: Testing Seed {seed} (Seat {seat}) ---")
    for mode in ["A", "B", "C", "D"]:
        env = kaggle_environments.make("kaggriculture", configuration={"episodeSteps": 720, "seed": seed})
        env.reset()
        agent_obj = EXP130Agent(mode=mode)
        ...
        print(f"  [Arm {mode}]: D.1=${r0:,.0f} | Opp=${r1:,.0f} | Delta=${delta:+,.0f} | Won={won} | MilkTrig={agent_obj.milk_triggers} | StrawTrig={agent_obj.straw_triggers} | Sample={agent_obj.trigger_details[:2]}")

def main():
    print("=" * 115)
    print("EXP130 3-SEED PROOF: SEQUENTIAL ENDGAME DEFENSE FACTORIZATION")
    print("=" * 115)
    test_single_seed(seed=1002, seat=0, label="Proof 1 (Milk Defense Firing)")
    test_single_seed(seed=1000, seat=0, label="Proof 2 (Strawberry Defense Firing)")
    test_single_seed(seed=42, seat=0, label="Proof 3 (Both Defenses Firing)")
