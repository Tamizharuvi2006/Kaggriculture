"""EXP154 Worker: Day-30 Labor Value Sweep (N=0..10) across Population Matches."""
...
def profile_match_state_and_sweep(seed: int, seat: int, b_key: str):
    opp_entry = POPULATION_SUITE[b_key]
    opp_fn = opp_entry["agent"]

    # 1. Run up to Step 696 with base D.1 to inspect Step 696 public state
    ...
    while not env.done:
        ...
        if step == 696:
            ...
            # Count unharvested ripe crops
            ripe_straw = sum(1 for r in tiles for t in r if isinstance(t, dict) and t.get("crop") == "STRAWBERRY" and t.get("age", 0) >= 3)
            unripe_straw = sum(1 for r in tiles for t in r if isinstance(t, dict) and t.get("crop") == "STRAWBERRY" and t.get("age", 0) < 3)
            ...
            # Potential unharvested field value
            field_potential = (ripe_straw * p_straw) + (ripe_wheat * p_wheat) + (cows * p_milk * 0.5)

            step696_state = {
                "money": money, "curr_workers": curr_workers,
                "ripe_straw": ripe_straw, "unripe_straw": unripe_straw,
                "ripe_wheat": ripe_wheat, "cows": cows,
    ...
    # 2. Sweep N in [0, 2, 4, 6, 8, 10] from game start
    sweep_rewards = {}
    for n in [0, 2, 4, 6, 8, 10]:
        agent_n = create_day30_agent(n)
        env_n = kaggle_environments.make("kaggriculture", configuration={"episodeSteps": 720, "seed": seed})
        env_n.reset()
        while not env_n.done:
            obs0 = env_n.state[0].observation if seat == 0 else env_n.state[1].observation
            obs1 = env_n.state[1].observation if seat == 0 else env_n.state[0].observation
            a0 = agent_n(obs0, env_n.configuration)
            try: a1 = opp_fn(obs1, env_n.configuration)
            except TypeError: a1 = opp_fn(obs1)
            env_n.step([a0, a1] if seat == 0 else [a1, a0])
        r0 = float(env_n.state[seat].reward or 0.0)
        opp_r = float(env_n.state[1 - seat].reward or 0.0)
        sweep_rewards[f"N_{n}"] = {"reward": r0, "opp": opp_r, "margin": r0 - opp_r, "won": r0 > opp_r}

    # Find optimal N*
    best_n = max([0, 2, 4, 6, 8, 10], key=lambda n: sweep_rewards[f"N_{n}"]["reward"])
...
def main():
    if len(sys.argv) < 3:
        print("Usage: python exp154_worker.py <bot_key_csv> <worker_id>")
        return
    ...
    for b_key in bot_keys:
        ...
        for i, seed in enumerate(seeds):
            ...
            res = profile_match_state_and_sweep(seed, seat, b_key)
            results.append(res)

    out_file = os.path.join(REPORTS_DIR, f"exp154_part_{worker_id}.json")
