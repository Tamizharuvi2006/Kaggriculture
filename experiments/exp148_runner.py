"""EXP148 Multi-Process Runner: Evaluates Arms A, B, C, D across the 5 benchmark suites."""
...
def main():
    print("=" * 145)
    print("EXP148: LAND #3 CAUSAL COUNTERFACTUAL & STRAWBERRY SATURATION BENCHMARK (400 TOTAL MATCHES)")
    print("=" * 145)
    ...
    for idx, bot_file in enumerate(OPPONENTS):
        ...
        cmd = [sys.executable, os.path.join(BASE_DIR, "experiments", "exp148_worker.py"), bot_file, worker_id]
    ...
    for idx, bot_file in enumerate(OPPONENTS):
        worker_id = f"opp_{idx}_{bot_file.split('.')[0]}"
        part_file = os.path.join(REPORTS_DIR, f"exp148_part_{worker_id}.json")
    ...
    opp_summaries = {}
    print("\n" + "=" * 145)
    print(f"{'Opponent Architecture':<30} | {'Arm A WR':<10} | {'Arm B WR':<10} | {'Arm C WR':<10} | {'Arm D WR':<10} | {'Delta B-A ($)':<16} | {'Delta D-A ($)'}")
    print("=" * 145)
    ...
    for opp_name in OPPONENTS:
        ...
        opp_summaries[opp_name] = {
            ...
            "delta_b": d_b, "delta_d": d_d,
        }

        print(f"{opp_name:<30} | {(n_a/n)*100:5.1f}%    | {(n_b/n)*100:5.1f}%    | {(n_c/n)*100:5.1f}%    | {(n_d/n)*100:5.1f}%    | ${d_b:+13,.2f}  | ${d_d:+13,.2f}")
    ...
    print(f"{'OVERALL (100 MATCHES / ARM)':<30} | {overall_wr_a:5.1f}%    | {overall_wr_b:5.1f}%    | {overall_wr_c:5.1f}%    | {overall_wr_d:5.1f}%    | ${mean_d_b:+13,.2f}  | ${mean_d_d:+13,.2f}")
    print("=" * 145)

    out_json = os.path.join(REPORTS_DIR, "exp148_land3_counterfactual_results.json")
    with open(out_json, "w", encoding="utf-8") as f:
        json.dump({
            "overall": {
                "wr_a": _to_native(overall_wr_a), "wr_b": _to_native(overall_wr_b),
                "wr_c": _to_native(overall_wr_c), "wr_d": _to_native(overall_wr_d),
    ...
    print(f"\nSaved Complete EXP148 Benchmark Results: {out_json}")
