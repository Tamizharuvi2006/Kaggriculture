"""EXP141 Multi-Process Runner: Evaluates Arms A, B, C, D across the 5 benchmark suites."""
...
def main():
    print("=" * 135)
    print("EXP141: EARLY CARE FACTORIAL BENCHMARK (400 TOTAL MATCHES ACROSS 5 PROCESSES)")
    print("=" * 135)
    ...
    for idx, bot_file in enumerate(OPPONENTS):
        ...
        cmd = [sys.executable, os.path.join(BASE_DIR, "experiments", "exp141_worker.py"), bot_file, worker_id]
    ...
    for idx, bot_file in enumerate(OPPONENTS):
        worker_id = f"opp_{idx}_{bot_file.split('.')[0]}"
        part_file = os.path.join(REPORTS_DIR, f"exp141_part_{worker_id}.json")
    ...
    opp_summaries = {}
    print("\n" + "=" * 135)
    print(f"{'Opponent Architecture':<35} | {'Arm A WR':<10} | {'Arm B WR':<10} | {'Arm C WR':<10} | {'Arm D WR':<10} | {'Delta B-A':<15} | {'Delta C-A':<15} | {'Delta D-A'}")
    print("=" * 135)

    total_matches = len(all_data)
    a_wins, b_wins, c_wins, d_wins = 0, 0, 0, 0
    tot_d_b, tot_d_c, tot_d_d = 0.0, 0.0, 0.0
    ...
    for opp_name in OPPONENTS:
        ...
        d_d = float(np.mean([d["delta_d_vs_a"] for d in sub_items]))
        ...
        opp_summaries[opp_name] = {
            "matches": n,
            "wr_a": (n_a / n) * 100, "wr_b": (n_b / n) * 100, "wr_c": (n_c / n) * 100, "wr_d": (n_d / n) * 100,
            "delta_b": d_b, "delta_c": d_c, "delta_d": d_d,
        }

        print(f"{opp_name:<35} | {(n_a/n)*100:5.1f}%     | {(n_b/n)*100:5.1f}%     | {(n_c/n)*100:5.1f}%     | {(n_d/n)*100:5.1f}%     | ${d_b:+13,.2f} | ${d_c:+13,.2f} | ${d_d:+13,.2f}")
    ...
    print(f"{'OVERALL (100 MATCHES / ARM)':<35} | {overall_wr_a:5.1f}%     | {overall_wr_b:5.1f}%     | {overall_wr_c:5.1f}%     | {overall_wr_d:5.1f}%     | ${mean_d_b:+13,.2f} | ${mean_d_c:+13,.2f} | ${mean_d_d:+13,.2f}")
    print("=" * 135)

    out_json = os.path.join(REPORTS_DIR, "exp141_care_factorial_results.json")
    with open(out_json, "w", encoding="utf-8") as f:
        json.dump({
            "overall": {
                ...
                "wr_d": _to_native(overall_wr_d),
                "mean_delta_b": _to_native(mean_d_b),
                "mean_delta_c": _to_native(mean_d_c),
                "mean_delta_d": _to_native(mean_d_d),
            },
            "by_opponent": _to_native(opp_summaries),
            "all_matches": _to_native(all_data),
        }, f, indent=2)

    print(f"\nSaved Complete EXP141 Benchmark Results: {out_json}")
