"""
EXP194 — Train Opponent-Conditioned Macro Q-Network on GPU from exp194_opponent_dataset.csv.
"""
...
CSV_PATH = r"D:\kaggriculture\data\exp194_opponent_dataset.csv"
MODEL_JSON = r"D:\kaggriculture\models\exp194_opponent_q_weights.json"
MODEL_NPZ = r"D:\kaggriculture\models\exp194_opponent_q_weights.npz"

class OpponentQNetwork(nn.Module):
    def __init__(self, state_dim=16, num_actions=6, hidden_dim=128):
        ...
        self.advantage_head = nn.Linear(64, num_actions) # ΔQ(s, a, opp) relative to a0
        self.policy_logits = nn.Linear(64, num_actions)  # P(best_a | s, opp)
...
def train():
    print("=" * 80)
    print("EXP194 -- TRAINING OPPONENT-CONDITIONED MACRO Q-NETWORK ON GPU")
    print("=" * 80)
    ...
    print(f"Loaded {len(df):,} opponent-conditioned records from EXP194 dataset.")
    ...
    model = OpponentQNetwork(state_dim=len(feature_cols), num_actions=6).to(device)
    optimizer = optim.AdamW(model.parameters(), lr=1e-3, weight_decay=1e-4)
    smooth_l1 = nn.SmoothL1Loss(beta=500.0)
    ce_loss = nn.CrossEntropyLoss()

    print("\n--- Training Opponent-Conditioned Q-Network (30 Epochs) ---")
    t0 = time.time()
