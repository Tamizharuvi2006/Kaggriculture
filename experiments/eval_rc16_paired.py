"""Paired seat-swapped evaluation: RC16 vs RC15 (current production).

Hypothesis: removing the Day-10 rich-opponent animal freeze lets the bot
...
RC16 = os.path.join(ROOT, "submission_rc16.py")
...
def _play(seed: int, cand_seat: int):
    agents = [RC16, RC15] if cand_seat == 0 else [RC15, RC16]
...
def main():
    ...
    out_path = os.path.join(ROOT, "reports", "RC16_PAIRED_VS_RC15.json")
