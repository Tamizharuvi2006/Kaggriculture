"""
Build Candidate Submission for EXP-0155 (CAND-155-02: Closed-Loop Seed Synchronization)
Modifies submission.py:
1. Implements Seed Synchronization + Worker #3 Post-Pasture Cultivation in _apply_fixed_board_adaptation():
   - Step 75: ['SELL', 'MELON', 6] + ['BUY_SEED', 'STRAWBERRY', 6]
   - Step 152: ['BUY_LAND']
   - Step 156: Synchronized Seed Purchase: Appends ['BUY_SEED', 'STRAWBERRY', 2] so 2 seeds remain in shed!
   - Step 159: Worker #2 & Worker #3 BUILD_PASTURE (100% Protected Milestone).
   - Steps 160-167: Worker #3 detours South, tills, PLANTS strawberry (consuming shed seed), waters, and returns North.
   - Step 170: Worker #0 Cow Pickup (100% Protected Milestone).
   - Step 171: Zero coordinate drift at schedule resumption.
Produces:
- apex_next/research/EXP-0155/candidate/candidate_submission.py
"""
import os
import sys
import json
...
def build_candidate():
    print("==========================================================================")
    print("[EXP-0155] BUILDING CANDIDATE SUBMISSION (CAND-155-02: SEED SYNCHRONIZATION)")
    print("==========================================================================\n")
    ...
    sync_hook = '''def _apply_fixed_board_adaptation(obs, action):
    """Closed-Loop Seed Synchronization with 100% Protected Critical Milestones."""
    ...
    # 2. SYNCHRONIZED SEED PURCHASE @ Step 156
    # Buys 2 extra strawberry seeds so Worker #3 can physically plant at Step 163!
    if step == 156:
        copied["market"].append(["BUY_SEED", "STRAWBERRY", 2])

    # 3. Worker #3 Post-Pasture Detour (Steps 160 to 167)
...
'''
    ...
    new_code = re.sub(pattern, sync_hook, code)
    
    out_dir = os.path.join(_PROJECT_ROOT, "apex_next", "research", "EXP-0155", "candidate")
