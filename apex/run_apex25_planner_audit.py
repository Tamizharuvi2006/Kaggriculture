"""APEX 2.5 Planner Diversity & Step-Range Telemetry Audit Benchmark.
"""
...
def run_planner_diversity_audit():
    print("====================================================================================================", flush=True)
    print("🚀 APEX 2.5 PLANNER DIVERSITY & STEP-RANGE TELEMETRY AUDIT", flush=True)
    print("====================================================================================================", flush=True)
    ...
    for seed in all_seeds:
        ...
        results.append((seed, ctrl_wealth, apex_wealth, margin, status))

        print(f"Seed {seed:<10} | L+: ${ctrl_wealth:10,.2f} | APEX 2.5: ${apex_wealth:10,.2f} | Margin: ${margin:+10,.2f} | {status}", flush=True)
    ...
    print("\n====================================================================================================", flush=True)
    print("🏆 APEX 2.5 PLANNER DIVERSITY & STEP-RANGE TELEMETRY AUDIT REPORT", flush=True)
    print("====================================================================================================", flush=True)
    ...
    print(f"  ├── GATE B (Safety & Zero-Regression): {'PASSED ✅' if all(r[4] == 'WIN ✅' for r in results) else 'FAILED ❌'}", flush=True)
    print(f"  ├── GATE C (Controlled Divergence)   : {'PASSED ✅' if metrics['divergence_rate_pct'] > 0 else 'MONITORED 🔍'}", flush=True)
    print("====================================================================================================", flush=True)

if __name__ == "__main__":
    run_planner_diversity_audit()
