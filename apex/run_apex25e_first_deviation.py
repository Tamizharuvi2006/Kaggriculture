"""APEX 2.5-E First Real Safe Deviation Benchmark.
"""
...
def run_first_deviation_benchmark():
    print("====================================================================================================", flush=True)
    print("🔥 APEX 2.5-E FIRST REAL SAFE DEVIATION BENCHMARK & OUTCOME ATTRIBUTION", flush=True)
    print("====================================================================================================", flush=True)
    ...
    for seed in all_seeds:
        ...
        # Reset APEX Policy for Clean Episode
        policy_inst = apex_mod._POLICY
        policy_inst.reset_episode()
        policy_inst.telemetry_traces.clear()
        policy_inst.divergences = 0

        # Run APEX 2.5-E Candidate
        ...
        results.append(res)
        print(f"Seed {seed:<10} | L+: ${ctrl_wealth:10,.2f} | APEX 2.5-E: ${apex_wealth:10,.2f} | Margin: ${margin:+10,.2f} | {status} | Deviations: {metrics['divergences']}", flush=True)
    ...
    total_deviations_across_episodes = sum(r['divergences'] for r in results)

    print("\n====================================================================================================", flush=True)
    print("🏆 APEX 2.5-E FIRST DEVIATION EXECUTION & OUTCOME REPORT", flush=True)
    print("====================================================================================================", flush=True)
    ...
    print(f"Executed Controlled Deviations   : {total_deviations_across_episodes} (Max 1 / Episode)", flush=True)
    print("----------------------------------------------------------------------------------------------------", flush=True)
    ...
    print("----------------------------------------------------------------------------------------------------", flush=True)
    print("GATE QUALIFICATION CHECKLIST:", flush=True)
    print(f"  ├── GATE A (Mid-Game Candidates > 0) : {'PASSED ✅' if audit['total_generated'] > 0 else 'FAILED ❌'}", flush=True)
    print(f"  ├── GATE B (Safety & Zero-Regression): {'PASSED ✅' if all(r['status'] == 'WIN ✅' for r in results) else 'FAILED ❌'}", flush=True)
    print(f"  ├── GATE C (First Deviation Executed): {'PASSED ✅' if total_deviations_across_episodes > 0 else 'FAILED ❌'}", flush=True)
    print("====================================================================================================", flush=True)

if __name__ == "__main__":
    run_first_deviation_benchmark()
