//! EXP193 — Generalized Q-Guided Macro Action Policy.
//! Combines Multi-Action Macro Q-Network with FastSim Full-Terminal Verification Guard.
...
pub struct EXP193MacroPolicy {
    name: &'static str,
    base_policy: AdaptiveTerminalPolicy,
    weights: MacroQWeights,
}

impl EXP193MacroPolicy {
    pub fn new() -> Self {
        let weights_json = include_str!("../../../models/exp193_macro_q_weights.json");
        let weights: MacroQWeights = serde_json::from_str(weights_json)
            .expect("Failed to parse exp193_macro_q_weights.json");

        Self {
            name: "exp193_macro_policy",
    ...
    pub fn forward(&self, features: &[f32; 10]) -> ([f32; 6], [f32; 6]) {
        let w = &self.weights;
...
impl Policy for EXP193MacroPolicy {
    ...
    fn act(&self, state: &GameState, player_idx: usize) -> PlayerAction {
        ...
        // Check for macro decision moments:
        // Moment 1: Day 0 Step 0 (Early wheat insurance)
        // Moment 2: Day 6 Hour 0 (Labor rescue)
        // Moment 3: Day 8 Hour 4 (Sheep sizing)
