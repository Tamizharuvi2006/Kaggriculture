//! EXP198 — Alpha Opportunity Policy.
//! Combines Two-Headed Alpha Opportunity Gate (P(Alpha) >= 0.70) with Scheduled Windows and FastSim Verification.
...
pub struct AlphaMinerWeights {
    ...
    pub alpha_gate_weight: Vec<Vec<f32>>,
    pub alpha_gate_bias: Vec<f32>,
    pub action_head_weight: Vec<Vec<f32>>,
    pub action_head_bias: Vec<f32>,
}

pub struct EXP198AlphaPolicy {
    name: &'static str,
    base_policy: AdaptiveTerminalPolicy,
    weights: AlphaMinerWeights,
}

impl EXP198AlphaPolicy {
    pub fn new() -> Self {
        let weights_json = include_str!("../../../models/exp198_alpha_miner_weights.json");
        let weights: AlphaMinerWeights = serde_json::from_str(weights_json)
            .expect("Failed to parse exp198_alpha_miner_weights.json");

        Self {
            name: "exp198_alpha_policy",
    ...
    /// Forward pass: returns (p_alpha_opportunity, action_logits)
    pub fn forward(&self, features: &[f32; 16]) -> (f32, [f32; 5]) {
        let w = &self.weights;
        ...
        // Alpha Gate: 64 -> 1 (Sigmoid)
        let mut gate_logit = w.alpha_gate_bias[0];
        for j in 0..64 { gate_logit += w.alpha_gate_weight[0][j] * h2[j]; }
        let p_alpha = 1.0 / (1.0 + (-gate_logit).exp());

        // Action Head: 64 -> 5 (Logits for a1..a5)
        let mut act_logits = [0.0f32; 5];
        for i in 0..5 {
...
impl Policy for EXP198AlphaPolicy {
    ...
    fn act(&self, state: &GameState, player_idx: usize) -> PlayerAction {
        ...
        if let Some(cands) = candidate_set {
            ...
            let (p_alpha, act_logits) = self.forward(&features);

            // Trigger intervention only when Alpha Gate indicates high conviction (P >= 0.70)
            if p_alpha >= 0.70 {
                ...
                // Verify the proposed alpha action with full lookahead
