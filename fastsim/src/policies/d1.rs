//! D.1 Control Policy (Tournament Baseline Control A).
...
const V18_RUNTIME_JSON: &str = include_str!("../../data/v41_full_runtime.json");
...
pub struct D1Policy {
    ...
    stay_bonus: f64,
    runtime_state: Mutex<D1RuntimeState>,
}

struct D1RuntimeState {
...
impl D1Policy {
    pub fn new() -> Self {
        let v: Value = serde_json::from_str(V18_RUNTIME_JSON)
            .expect("Failed to parse embedded V18 runtime JSON");

        let b0 = v["board_by_seat"]["0"].as_str().unwrap_or("senkin").to_string();
        ...
        if let Some(exp_map) = v["experts"].as_object() {
            for (name, exp_val) in exp_map {
                let mut actions = Vec::new();
                if let Some(act_list) = exp_val["actions"].as_array() {
                    for act_obj in act_list {
                        actions.push(parse_json_action(act_obj));
                    }
                }

                let mut prototypes_by_day = Vec::new();
                if let Some(proto_list) = exp_val["prototypes_by_day"].as_array() {
                    for p_arr in proto_list {
                        let proto: Vec<f64> = p_arr.as_array().unwrap()
                            .iter().map(|x| x.as_f64().unwrap_or(0.0)).collect();
                        prototypes_by_day.push(proto);
                    }
                }
        ...
        Self {
            ...
            distance_strength,
            stay_bonus,
            runtime_state: Mutex::new(D1RuntimeState {
    ...
    fn act(&self, state: &GameState, player_idx: usize) -> PlayerAction {
        ...
        if rt.selected_day[player_idx] != Some(day) || rt.selected_market[player_idx].is_none() {
            ...
            for (name, expert) in &self.experts {
                ...
                let b = *bias.get(name).unwrap_or(&0.0);
                let score = b - self.distance_strength * distance + stay;
        ...
        PlayerAction {
            ...
            market: market_act.market.clone(),
        }
    }
}
