//! EXP188 — 10,000-Match AdaptiveGuarded Tournament against AdaptiveTerminal & D.1.
//! Evaluates elimination of the Day 8 Over-Expansion Liquidity Trap.

use fastsim::engine::state::GameState;
use fastsim::engine::step::step_game;
use fastsim::policies::{Policy, AdaptiveTerminalPolicy, AdaptiveGuardedPolicy, D1Policy};
use rayon::prelude::*;
...
fn main() {
    println!("=========================================================================================");
    println!("     EXP188 TOURNAMENT: ADAPTIVE-GUARDED vs ADAPTIVE TERMINAL (10,000 MATCHES)           ");
    println!("=========================================================================================");

    let seeds: Vec<u64> = (1000..6000).collect(); // 5,000 diverse seeds x 2 seats = 10,000 matches
    let t0 = Instant::now();

    // 1. Guarded (Seat 0) vs Adaptive (Seat 1)
    println!("\n[1/2] Guarded (Seat 0) vs Adaptive (Seat 1)...");
    let stats_g_vs_a_s0 = evaluate_matchup(AdaptiveGuardedPolicy::new, AdaptiveTerminalPolicy::new, "Guarded", "Adaptive", &seeds);

    // 2. Adaptive (Seat 0) vs Guarded (Seat 1)
    println!("[2/2] Adaptive (Seat 0) vs Guarded (Seat 1)...");
    let stats_g_vs_a_s1 = evaluate_matchup(AdaptiveTerminalPolicy::new, AdaptiveGuardedPolicy::new, "Adaptive", "Guarded", &seeds);
    ...
    println!("=========================================================================================");
    print_stats(&stats_g_vs_a_s0);
    print_stats(&stats_g_vs_a_s1);

    let combined_g_wins = stats_g_vs_a_s0.p0_wins + stats_g_vs_a_s1.p1_wins;
    let combined_a_wins = stats_g_vs_a_s0.p1_wins + stats_g_vs_a_s1.p0_wins;
    let combined_ties = stats_g_vs_a_s0.ties + stats_g_vs_a_s1.ties;
    let total_matches = seeds.len() * 2;

    let g_win_pct = (combined_g_wins as f64 / total_matches as f64) * 100.0;
    let a_win_pct = (combined_a_wins as f64 / total_matches as f64) * 100.0;
    let combined_delta = ((stats_g_vs_a_s0.p0_mean - stats_g_vs_a_s0.p1_mean) + (stats_g_vs_a_s1.p1_mean - stats_g_vs_a_s1.p0_mean)) / 2.0;

    println!("=========================================================================================");
    println!("                         SEAT-BALANCED COMBINED SCORECARD                                ");
    println!("=========================================================================================");
    println!("Adaptive-Guarded Win Rate    : {:.2}% ({}/{})", g_win_pct, combined_g_wins, total_matches);
