//! EXP205 — 10,000-Match Head-to-Head Tournament & Multi-Tier Population Gate for EXP205 Elite Frontier Policy.

use fastsim::engine::state::GameState;
use fastsim::engine::step::step_game;
use fastsim::policies::{
    Policy, AdaptiveTerminalPolicy, EXP205FrontierPolicy, D1Policy, V41Policy, AgroHybridPolicy
};
use rayon::prelude::*;
...
fn main() {
    println!("=========================================================================================================================");
    println!("     EXP205 — 10,000-MATCH TOURNAMENT & MULTI-TIER POPULATION GATE (EXP205 ELITE FRONTIER CANDIDATE #1)                  ");
    println!("=========================================================================================================================");

    let h2h_seeds: Vec<u64> = (800000..805000).collect(); // 5,000 fresh seeds x 2 seats = 10,000 matches
    let t0 = Instant::now();

    println!("\n[1/6] EXP205-Frontier (Seat 0) vs Adaptive-Terminal (Seat 1)...");
    let stats_a_vs_a_s0 = evaluate_matchup(EXP205FrontierPolicy::new, AdaptiveTerminalPolicy::new, "EXP205-Frontier", "Adaptive", &h2h_seeds);

    println!("[2/6] Adaptive-Terminal (Seat 0) vs EXP205-Frontier (Seat 1)...");
    let stats_a_vs_a_s1 = evaluate_matchup(AdaptiveTerminalPolicy::new, EXP205FrontierPolicy::new, "Adaptive", "EXP205-Frontier", &h2h_seeds);
    ...
    println!("=========================================================================================================================");
    println!("EXP205-Frontier Combined Win Rate: {:.2}% ({}/{})", a0_win_pct, combined_a0_wins, total_h2h);
    ...
    println!("EXP205 Mean Reward               : ${:.1} (vs Adaptive Mean: ${:.1})", (stats_a_vs_a_s0.p0_mean + stats_a_vs_a_s1.p1_mean)/2.0, (stats_a_vs_a_s0.p1_mean + stats_a_vs_a_s1.p0_mean)/2.0);
    println!("Net Margin Delta (Hero - Opp)    : {:+.2}", combined_delta);
    println!("EXP205 Worst 5% Floor            : ${:.1} (vs Adaptive Floor: ${:.1})", stats_a_vs_a_s0.p0_worst_5pct, stats_a_vs_a_s0.p1_worst_5pct);
    println!("EXP205 Worst 1% Floor            : ${:.1} (vs Adaptive Floor: ${:.1})", stats_a_vs_a_s0.p0_worst_1pct, stats_a_vs_a_s0.p1_worst_1pct);
    println!("=========================================================================================================================");

    println!("\n--- RUNNING MULTI-TIER POPULATION PROMOTION GATE (16,000 MATCHES) ---");
    let tier_seeds: Vec<u64> = (805000..807000).collect();
    ...
    println!("=========================================================================================================================");
    println!("                                   EXP205 MULTI-TIER SCORECARD                                                           ");
    println!("=========================================================================================================================");
