//! Test and benchmark MultiCropPlannerPolicy vs AdaptiveTerminal in FastSim.

use fastsim::engine::state::GameState;
use fastsim::engine::step::step_game;
use fastsim::policies::{AdaptiveTerminalPolicy, MultiCropPlannerPolicy, Policy};
use rayon::prelude::*;
use std::time::Instant;

fn main() {
    println!("================================================================================");
    println!("EXP179: BENCHMARKING NATIVE MULTI-CROP DYNAMIC MOTION PLANNER VS ADAPTIVE CONTROL");
    println!("================================================================================");

    let planner = MultiCropPlannerPolicy::new();
    let control = AdaptiveTerminalPolicy::new();

    let seeds: Vec<u64> = (1000..2000).collect(); // 1,000 seeds x 2 seats = 2,000 matches
    ...
    println!("Simulating {} paired matches (Planner vs Adaptive Control)...", tasks.len());
    let t0 = Instant::now();
    ...
    let results: Vec<(f64, f64, [f64; 6])> = tasks.into_par_iter().map(|(seed, seat)| {
        ...
        while !state.done && state.step < 720 {
            ...
            let a_hero = planner.act(&state, seat);
    ...
    println!("    Mean Planner Reward: ${:7.1} vs Ctrl: ${:7.1} | Delta: {:+6.1}", mean_hero, mean_ctrl, mean_hero - mean_ctrl);
    println!("    Planner Cash Curve : D1=${:.0}, D4=${:.0}, D7=${:.0}, D11=${:.0}, D15=${:.0}, D30=${:.0}",
