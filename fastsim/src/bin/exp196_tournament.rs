//! EXP196 — 10,000-Match Head-to-Head Tournament & Multi-Tier Population Gate for EXP196 Trinary Policy.

use fastsim::engine::state::GameState;
use fastsim::engine::step::step_game;
use fastsim::policies::{
    Policy, AdaptiveTerminalPolicy, EXP196TrinaryPolicy, D1Policy, V41Policy, AgroHybridPolicy
};
use rayon::prelude::*;
    ...
    Opp: Policy + 'static,
{
    ...
    let s0_res: Vec<(f64, f64)> = seeds.par_iter().map(|&seed| {
        let hero = EXP196TrinaryPolicy::new();
        let opp = opp_factory();
...
fn main() {
    println!("=========================================================================================================================");
    println!("     EXP196 — 10,000-MATCH TOURNAMENT & MULTI-TIER POPULATION GATE (EXP196 TRINARY POLICY)                               ");
    println!("=========================================================================================================================");

    let h2h_seeds: Vec<u64> = (140000..145000).collect(); // 5,000 fresh seeds x 2 seats = 10,000 matches
    let t0 = Instant::now();

    // 1. EXP196 Trinary (Seat 0) vs Adaptive (Seat 1)
    println!("\n[1/6] EXP196-Trinary (Seat 0) vs Adaptive-Terminal (Seat 1)...");
    let stats_t_vs_a_s0 = evaluate_matchup(EXP196TrinaryPolicy::new, AdaptiveTerminalPolicy::new, "EXP196-Trinary", "Adaptive", &h2h_seeds);

    // 2. Adaptive (Seat 0) vs EXP196 Trinary (Seat 1)
    println!("[2/6] Adaptive-Terminal (Seat 0) vs EXP196-Trinary (Seat 1)...");
    let stats_t_vs_a_s1 = evaluate_matchup(AdaptiveTerminalPolicy::new, EXP196TrinaryPolicy::new, "Adaptive", "EXP196-Trinary", &h2h_seeds);
    ...
    println!("=========================================================================================================================");
    println!("EXP196-Trinary Combined Win Rate : {:.2}% ({}/{})", t_win_pct, combined_t_wins, total_h2h);
    ...
    println!("EXP196 Worst 5% Floor            : ${:.1} (vs Adaptive Floor: ${:.1})", stats_t_vs_a_s0.p0_worst_5pct, stats_t_vs_a_s0.p1_worst_5pct);
    println!("EXP196 Worst 1% Floor            : ${:.1} (vs Adaptive Floor: ${:.1})", stats_t_vs_a_s0.p0_worst_1pct, stats_t_vs_a_s0.p1_worst_1pct);
    println!("=========================================================================================================================");

    // Multi-Tier Population Promotion Gate
    println!("\n--- RUNNING MULTI-TIER POPULATION PROMOTION GATE (16,000 MATCHES) ---");
    let tier_seeds: Vec<u64> = (145000..147000).collect(); // 2,000 held-out seeds x 2 seats x 4 tiers
    ...
    println!("=========================================================================================================================");
    println!("                                   EXP196 MULTI-TIER SCORECARD                                                           ");
    println!("=========================================================================================================================");
