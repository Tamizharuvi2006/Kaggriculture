//! EXP194 — 10,000-Match Head-to-Head Tournament & Multi-Tier Population Gate for EXP194 Opponent-Conditioned Policy.

use fastsim::engine::state::GameState;
use fastsim::engine::step::step_game;
use fastsim::policies::{
    Policy, AdaptiveTerminalPolicy, EXP194OpponentPolicy, D1Policy, V41Policy, AgroHybridPolicy
};
use rayon::prelude::*;
    ...
    Opp: Policy + 'static,
{
    ...
    let s0_res: Vec<(f64, f64)> = seeds.par_iter().map(|&seed| {
        let hero = EXP194OpponentPolicy::new();
        let opp = opp_factory();
...
fn main() {
    println!("=========================================================================================================================");
    println!("     EXP194 — 10,000-MATCH TOURNAMENT & MULTI-TIER POPULATION GATE (EXP194 OPPONENT POLICY)                              ");
    println!("=========================================================================================================================");

    let h2h_seeds: Vec<u64> = (110000..115000).collect(); // 5,000 fresh seeds x 2 seats = 10,000 matches
    let t0 = Instant::now();

    // 1. EXP194 Opponent (Seat 0) vs Adaptive (Seat 1)
    println!("\n[1/6] EXP194-Opponent (Seat 0) vs Adaptive-Terminal (Seat 1)...");
    let stats_o_vs_a_s0 = evaluate_matchup(EXP194OpponentPolicy::new, AdaptiveTerminalPolicy::new, "EXP194-Opponent", "Adaptive", &h2h_seeds);

    // 2. Adaptive (Seat 0) vs EXP194 Opponent (Seat 1)
    println!("[2/6] Adaptive-Terminal (Seat 0) vs EXP194-Opponent (Seat 1)...");
    let stats_o_vs_a_s1 = evaluate_matchup(AdaptiveTerminalPolicy::new, EXP194OpponentPolicy::new, "Adaptive", "EXP194-Opponent", &h2h_seeds);

    let combined_o_wins = stats_o_vs_a_s0.p0_wins + stats_o_vs_a_s1.p1_wins;
    let combined_a_wins = stats_o_vs_a_s0.p1_wins + stats_o_vs_a_s1.p0_wins;
    let combined_ties = stats_o_vs_a_s0.ties + stats_o_vs_a_s1.ties;
    let total_h2h = h2h_seeds.len() * 2;

    let o_win_pct = (combined_o_wins as f64 / total_h2h as f64) * 100.0;
    let a_win_pct = (combined_a_wins as f64 / total_h2h as f64) * 100.0;
    let combined_delta = ((stats_o_vs_a_s0.p0_mean - stats_o_vs_a_s0.p1_mean) + (stats_o_vs_a_s1.p1_mean - stats_o_vs_a_s1.p0_mean)) / 2.0;

    println!("\n=========================================================================================================================");
    println!("                                   HEAD-TO-HEAD COMBINED SCORECARD (vs Adaptive)                                         ");
    println!("=========================================================================================================================");
    println!("EXP194-Opponent Combined Win Rate: {:.2}% ({}/{})", o_win_pct, combined_o_wins, total_h2h);
    ...
    println!("EXP194 Worst 5% Floor            : ${:.1} (vs Adaptive Floor: ${:.1})", stats_o_vs_a_s0.p0_worst_5pct, stats_o_vs_a_s0.p1_worst_5pct);
    println!("EXP194 Worst 1% Floor            : ${:.1} (vs Adaptive Floor: ${:.1})", stats_o_vs_a_s0.p0_worst_1pct, stats_o_vs_a_s0.p1_worst_1pct);
    println!("=========================================================================================================================");

    // Multi-Tier Population Promotion Gate
    println!("\n--- RUNNING MULTI-TIER POPULATION PROMOTION GATE (16,000 MATCHES) ---");
    let tier_seeds: Vec<u64> = (115000..117000).collect(); // 2,000 held-out seeds x 2 seats x 4 tiers
    ...
    println!("=========================================================================================================================");
    println!("                                   EXP194 MULTI-TIER SCORECARD                                                           ");
    println!("=========================================================================================================================");
