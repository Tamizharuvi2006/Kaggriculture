//! EXP200 — 10,000-Match Head-to-Head Tournament & Multi-Tier Population Gate for EXP200 Competitive Policy.

use fastsim::engine::state::GameState;
use fastsim::engine::step::step_game;
use fastsim::policies::{
    Policy, AdaptiveTerminalPolicy, EXP200CompetitivePolicy, D1Policy, V41Policy, AgroHybridPolicy
};
use rayon::prelude::*;
    ...
    Opp: Policy + 'static,
{
    let s0_res: Vec<(f64, f64)> = seeds.par_iter().map(|&seed| {
        let hero = EXP200CompetitivePolicy::new();
        let opp = opp_factory();
...
fn main() {
    println!("=========================================================================================================================");
    println!("     EXP200 — 10,000-MATCH TOURNAMENT & MULTI-TIER POPULATION GATE (EXP200 COMPETITIVE POLICY)                           ");
    println!("=========================================================================================================================");

    let h2h_seeds: Vec<u64> = (210000..215000).collect(); // 5,000 fresh seeds x 2 seats = 10,000 matches
    let t0 = Instant::now();

    println!("\n[1/6] EXP200-Competitive (Seat 0) vs Adaptive-Terminal (Seat 1)...");
    let stats_a_vs_a_s0 = evaluate_matchup(EXP200CompetitivePolicy::new, AdaptiveTerminalPolicy::new, "EXP200-Competitive", "Adaptive", &h2h_seeds);

    println!("[2/6] Adaptive-Terminal (Seat 0) vs EXP200-Competitive (Seat 1)...");
    let stats_a_vs_a_s1 = evaluate_matchup(AdaptiveTerminalPolicy::new, EXP200CompetitivePolicy::new, "Adaptive", "EXP200-Competitive", &h2h_seeds);
    ...
    println!("=========================================================================================================================");
    println!("EXP200-Competitive Combined Win Rate: {:.2}% ({}/{})", a0_win_pct, combined_a0_wins, total_h2h);
    ...
    println!("EXP200 Worst 5% Floor               : ${:.1} (vs Adaptive Floor: ${:.1})", stats_a_vs_a_s0.p0_worst_5pct, stats_a_vs_a_s0.p1_worst_5pct);
    println!("EXP200 Worst 1% Floor               : ${:.1} (vs Adaptive Floor: ${:.1})", stats_a_vs_a_s0.p0_worst_1pct, stats_a_vs_a_s0.p1_worst_1pct);
    println!("=========================================================================================================================");

    println!("\n--- RUNNING MULTI-TIER POPULATION PROMOTION GATE (16,000 MATCHES) ---");
    let tier_seeds: Vec<u64> = (215000..217000).collect();
    ...
    println!("=========================================================================================================================");
    println!("                                   EXP200 MULTI-TIER SCORECARD                                                           ");
    println!("=========================================================================================================================");
