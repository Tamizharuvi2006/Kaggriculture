//! EXP193 — 10,000-Match Head-to-Head Tournament & Multi-Tier Population Gate for EXP193 Macro Policy.

use fastsim::engine::state::GameState;
use fastsim::engine::step::step_game;
use fastsim::policies::{
    Policy, AdaptiveTerminalPolicy, EXP193MacroPolicy, D1Policy, V41Policy, AgroHybridPolicy
};
use rayon::prelude::*;
    ...
    Opp: Policy + 'static,
{
    ...
    let s0_res: Vec<(f64, f64)> = seeds.par_iter().map(|&seed| {
        let hero = EXP193MacroPolicy::new();
        let opp = opp_factory();
...
fn main() {
    println!("=========================================================================================================================");
    println!("     EXP193 — 10,000-MATCH TOURNAMENT & MULTI-TIER POPULATION GATE (EXP193 MACRO POLICY)                                 ");
    println!("=========================================================================================================================");

    let h2h_seeds: Vec<u64> = (90000..95000).collect(); // 5,000 fresh seeds x 2 seats = 10,000 matches
    let t0 = Instant::now();

    // 1. EXP193 Macro (Seat 0) vs Adaptive (Seat 1)
    println!("\n[1/6] EXP193-Macro (Seat 0) vs Adaptive-Terminal (Seat 1)...");
    let stats_m_vs_a_s0 = evaluate_matchup(EXP193MacroPolicy::new, AdaptiveTerminalPolicy::new, "EXP193-Macro", "Adaptive", &h2h_seeds);

    // 2. Adaptive (Seat 0) vs EXP193 Macro (Seat 1)
    println!("[2/6] Adaptive-Terminal (Seat 0) vs EXP193-Macro (Seat 1)...");
    let stats_m_vs_a_s1 = evaluate_matchup(AdaptiveTerminalPolicy::new, EXP193MacroPolicy::new, "Adaptive", "EXP193-Macro", &h2h_seeds);

    let combined_m_wins = stats_m_vs_a_s0.p0_wins + stats_m_vs_a_s1.p1_wins;
    let combined_a_wins = stats_m_vs_a_s0.p1_wins + stats_m_vs_a_s1.p0_wins;
    let combined_ties = stats_m_vs_a_s0.ties + stats_m_vs_a_s1.ties;
    let total_h2h = h2h_seeds.len() * 2;

    let m_win_pct = (combined_m_wins as f64 / total_h2h as f64) * 100.0;
    let a_win_pct = (combined_a_wins as f64 / total_h2h as f64) * 100.0;
    let combined_delta = ((stats_m_vs_a_s0.p0_mean - stats_m_vs_a_s0.p1_mean) + (stats_m_vs_a_s1.p1_mean - stats_m_vs_a_s1.p0_mean)) / 2.0;

    println!("\n=========================================================================================================================");
    println!("                                   HEAD-TO-HEAD COMBINED SCORECARD (vs Adaptive)                                         ");
    println!("=========================================================================================================================");
    println!("EXP193-Macro Combined Win Rate   : {:.2}% ({}/{})", m_win_pct, combined_m_wins, total_h2h);
    ...
    println!("EXP193 Worst 5% Floor            : ${:.1} (vs Adaptive Floor: ${:.1})", stats_m_vs_a_s0.p0_worst_5pct, stats_m_vs_a_s0.p1_worst_5pct);
    println!("EXP193 Worst 1% Floor            : ${:.1} (vs Adaptive Floor: ${:.1})", stats_m_vs_a_s0.p0_worst_1pct, stats_m_vs_a_s0.p1_worst_1pct);
    println!("=========================================================================================================================");

    // Multi-Tier Population Promotion Gate
    println!("\n--- RUNNING MULTI-TIER POPULATION PROMOTION GATE (16,000 MATCHES) ---");
    let tier_seeds: Vec<u64> = (95000..97000).collect(); // 2,000 held-out seeds x 2 seats x 4 tiers
    ...
    println!("=========================================================================================================================");
    println!("                                   EXP193 MULTI-TIER SCORECARD                                                           ");
    println!("=========================================================================================================================");
