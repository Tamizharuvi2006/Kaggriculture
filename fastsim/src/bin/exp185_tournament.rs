//! EXP185 — 10,000-Match Dual-Head Verified Policy Tournament against AdaptiveTerminal & D.1.
//! Evaluates Tail-Risk Elimination, Win Rate, Mean Delta, and Cash Trajectories.

use fastsim::engine::state::GameState;
use fastsim::engine::step::step_game;
use fastsim::policies::{Policy, AdaptiveTerminalPolicy, EXP185VerifiedPolicy, D1Policy};
use rayon::prelude::*;
...
fn print_stats(stats: &MatchupStats) {
    ...
    println!("Worst 1% Floor: {} = ${:.1} | {} = ${:.1}",
        stats.p0_name, stats.p0_worst_1pct, stats.p1_name, stats.p1_worst_1pct);
...
fn main() {
    println!("=========================================================================================");
    println!("    EXP185 TOURNAMENT: DUAL-HEAD VERIFIED POLICY vs ADAPTIVE TERMINAL (10,000 MATCHES)   ");
    println!("=========================================================================================");

    let seeds: Vec<u64> = (1000..3500).collect(); // 2,500 diverse golden seeds x 2 seats = 5,000 matches
    let t0 = Instant::now();

    // 1. EXP185 Verified vs AdaptiveTerminal (Seat 0 vs Seat 1)
    println!("\n[1/4] EXP185 Verified (Seat 0) vs AdaptiveTerminal (Seat 1)...");
    let stats_v_vs_adap_s0 = evaluate_matchup(EXP185VerifiedPolicy::new, AdaptiveTerminalPolicy::new, "EXP185-Verified", "Adaptive", &seeds);

    // 2. AdaptiveTerminal vs EXP185 Verified (Seat 0 vs Seat 1 - Seat Swapped)
    println!("[2/4] AdaptiveTerminal (Seat 0) vs EXP185 Verified (Seat 1)...");
    let stats_v_vs_adap_s1 = evaluate_matchup(AdaptiveTerminalPolicy::new, EXP185VerifiedPolicy::new, "Adaptive", "EXP185-Verified", &seeds);

    // 3. EXP185 Verified vs D.1 (Seat Balanced)
    println!("[3/4] EXP185 Verified (Seat 0) vs D.1 (Seat 1)...");
    let stats_v_vs_d1_s0 = evaluate_matchup(EXP185VerifiedPolicy::new, D1Policy::new, "EXP185-Verified", "D.1", &seeds);

    println!("[4/4] D.1 (Seat 0) vs EXP185 Verified (Seat 1)...");
    let stats_v_vs_d1_s1 = evaluate_matchup(D1Policy::new, EXP185VerifiedPolicy::new, "D.1", "EXP185-Verified", &seeds);
    ...
    println!("=========================================================================================");
    print_stats(&stats_v_vs_adap_s0);
    print_stats(&stats_v_vs_adap_s1);
    print_stats(&stats_v_vs_d1_s0);
    print_stats(&stats_v_vs_d1_s1);

    let combined_v_wins = stats_v_vs_adap_s0.p0_wins + stats_v_vs_adap_s1.p1_wins;
    let combined_adap_wins = stats_v_vs_adap_s0.p1_wins + stats_v_vs_adap_s1.p0_wins;
    let combined_ties = stats_v_vs_adap_s0.ties + stats_v_vs_adap_s1.ties;
    ...
    let combined_delta = ((stats_v_vs_adap_s0.p0_mean - stats_v_vs_adap_s0.p1_mean) + (stats_v_vs_adap_s1.p1_mean - stats_v_vs_adap_s1.p0_mean)) / 2.0;

    println!("=========================================================================================");
    println!("                         SEAT-BALANCED COMBINED SCORECARD (vs Adaptive)                  ");
    println!("=========================================================================================");
    println!("EXP185-Verified Combined Win Rate: {:.2}% ({}/{})", v_win_pct, combined_v_wins, total_matches);
