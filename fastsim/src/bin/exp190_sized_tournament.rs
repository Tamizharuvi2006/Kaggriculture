//! EXP190 — 10,000-Match AdaptiveSized Tournament against AdaptiveTerminal & D.1.
//! Validates the State-Conditioned Livestock Sizing Governor derived in EXP189.

use fastsim::engine::state::GameState;
use fastsim::engine::step::step_game;
use fastsim::policies::{Policy, AdaptiveTerminalPolicy, AdaptiveSizedPolicy, D1Policy};
use rayon::prelude::*;
...
fn main() {
    println!("=========================================================================================");
    println!("     EXP190 TOURNAMENT: ADAPTIVE-SIZED vs ADAPTIVE TERMINAL (10,000 MATCHES)             ");
    println!("=========================================================================================");

    let seeds: Vec<u64> = (1000..6000).collect(); // 5,000 diverse seeds x 2 seats = 10,000 matches
    let t0 = Instant::now();

    // 1. Sized (Seat 0) vs Adaptive (Seat 1)
    println!("\n[1/4] Adaptive-Sized (Seat 0) vs Adaptive-Terminal (Seat 1)...");
    let stats_s_vs_a_s0 = evaluate_matchup(AdaptiveSizedPolicy::new, AdaptiveTerminalPolicy::new, "Adaptive-Sized", "Adaptive", &seeds);

    // 2. Adaptive (Seat 0) vs Sized (Seat 1)
    println!("[2/4] Adaptive-Terminal (Seat 0) vs Adaptive-Sized (Seat 1)...");
    let stats_s_vs_a_s1 = evaluate_matchup(AdaptiveTerminalPolicy::new, AdaptiveSizedPolicy::new, "Adaptive", "Adaptive-Sized", &seeds);

    // 3. Sized vs D.1 Grandmaster
    println!("[3/4] Adaptive-Sized (Seat 0) vs D.1 Grandmaster (Seat 1)...");
    let stats_s_vs_d1_s0 = evaluate_matchup(AdaptiveSizedPolicy::new, D1Policy::new, "Adaptive-Sized", "D.1", &seeds);

    println!("[4/4] D.1 Grandmaster (Seat 0) vs Adaptive-Sized (Seat 1)...");
    let stats_s_vs_d1_s1 = evaluate_matchup(D1Policy::new, AdaptiveSizedPolicy::new, "D.1", "Adaptive-Sized", &seeds);
    ...
    println!("=========================================================================================");
    print_stats(&stats_s_vs_a_s0);
    print_stats(&stats_s_vs_a_s1);
    print_stats(&stats_s_vs_d1_s0);
    print_stats(&stats_s_vs_d1_s1);

    let combined_s_wins = stats_s_vs_a_s0.p0_wins + stats_s_vs_a_s1.p1_wins;
    let combined_a_wins = stats_s_vs_a_s0.p1_wins + stats_s_vs_a_s1.p0_wins;
    let combined_ties = stats_s_vs_a_s0.ties + stats_s_vs_a_s1.ties;
    let total_matches = seeds.len() * 2;

    let s_win_pct = (combined_s_wins as f64 / total_matches as f64) * 100.0;
    let a_win_pct = (combined_a_wins as f64 / total_matches as f64) * 100.0;
    let combined_delta = ((stats_s_vs_a_s0.p0_mean - stats_s_vs_a_s0.p1_mean) + (stats_s_vs_a_s1.p1_mean - stats_s_vs_a_s1.p0_mean)) / 2.0;

    println!("=========================================================================================");
    println!("                         SEAT-BALANCED COMBINED SCORECARD (vs Adaptive)                  ");
    println!("=========================================================================================");
    println!("Adaptive-Sized Win Rate      : {:.2}% ({}/{})", s_win_pct, combined_s_wins, total_matches);
