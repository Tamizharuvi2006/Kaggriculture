//! EXP197 — High-Throughput Contrastive Macro Dataset Generator.
//! Evaluates 5,000 diverse seeds across candidate actions (a0..a5) with scheduled decision windows.
...
pub fn evaluate_contrastive_seed(seed: u64, opp_type: usize) -> ContrastiveSample {
    ...
    // 2. Evaluate a1: Buy Wheat at Day 0
...
fn main() {
    println!("=========================================================================================");
    println!("     EXP197 — CONTRASTIVE RESIDUAL-Q DATASET GENERATOR (10,000 SAMPLES)                  ");
    println!("=========================================================================================");

    let num_seeds = 2500;
    let seeds: Vec<u64> = (150000..(150000 + num_seeds as u64)).collect();
    let t0 = Instant::now();

    println!("Mining 2,500 seeds x 4 opponents x 6 actions = 60,000 full game rollouts...");
    ...
    let samples: Vec<ContrastiveSample> = tasks.into_par_iter().map(|(s, opp)| {
        evaluate_contrastive_seed(s, opp)
    }).collect();

    let elapsed = t0.elapsed().as_secs_f64();
    println!("Contrastive dataset generated in {:.2}s ({:.1} rollouts/sec)\n", elapsed, (num_seeds * 4 * 6) as f64 / elapsed);

    let csv_path = r"D:\kaggriculture\data\exp197_contrastive_dataset.csv";
    ...
    println!("Saved 10,000 contrastive samples to {}", csv_path);
}
