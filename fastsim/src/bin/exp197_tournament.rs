//! EXP197 — 10,000-Match Head-to-Head Tournament & Multi-Tier Population Gate for EXP197 Contrastive Policy.

use fastsim::engine::state::GameState;
use fastsim::engine::step::step_game;
use fastsim::policies::{
    Policy, AdaptiveTerminalPolicy, EXP197ContrastivePolicy, D1Policy, V41Policy, AgroHybridPolicy
};
use rayon::prelude::*;
    ...
    Opp: Policy + 'static,
{
    ...
    let s0_res: Vec<(f64, f64)> = seeds.par_iter().map(|&seed| {
        let hero = EXP197ContrastivePolicy::new();
        let opp = opp_factory();
...
fn main() {
    println!("=========================================================================================================================");
    println!("     EXP197 — 10,000-MATCH TOURNAMENT & MULTI-TIER POPULATION GATE (EXP197 CONTRASTIVE POLICY)                           ");
    println!("=========================================================================================================================");

    let h2h_seeds: Vec<u64> = (160000..165000).collect(); // 5,000 fresh seeds x 2 seats = 10,000 matches
    let t0 = Instant::now();

    // 1. EXP197 Contrastive (Seat 0) vs Adaptive (Seat 1)
    println!("\n[1/6] EXP197-Contrastive (Seat 0) vs Adaptive-Terminal (Seat 1)...");
    let stats_c_vs_a_s0 = evaluate_matchup(EXP197ContrastivePolicy::new, AdaptiveTerminalPolicy::new, "EXP197-Contrastive", "Adaptive", &h2h_seeds);

    // 2. Adaptive (Seat 0) vs EXP197 Contrastive (Seat 1)
    println!("[2/6] Adaptive-Terminal (Seat 0) vs EXP197-Contrastive (Seat 1)...");
    let stats_c_vs_a_s1 = evaluate_matchup(AdaptiveTerminalPolicy::new, EXP197ContrastivePolicy::new, "Adaptive", "EXP197-Contrastive", &h2h_seeds);

    let combined_c_wins = stats_c_vs_a_s0.p0_wins + stats_c_vs_a_s1.p1_wins;
    let combined_a_wins = stats_c_vs_a_s0.p1_wins + stats_c_vs_a_s1.p0_wins;
    let combined_ties = stats_c_vs_a_s0.ties + stats_c_vs_a_s1.ties;
    let total_h2h = h2h_seeds.len() * 2;

    let c_win_pct = (combined_c_wins as f64 / total_h2h as f64) * 100.0;
    let a_win_pct = (combined_a_wins as f64 / total_h2h as f64) * 100.0;
    let combined_delta = ((stats_c_vs_a_s0.p0_mean - stats_c_vs_a_s0.p1_mean) + (stats_c_vs_a_s1.p1_mean - stats_c_vs_a_s1.p0_mean)) / 2.0;

    println!("\n=========================================================================================================================");
    println!("                                   HEAD-TO-HEAD COMBINED SCORECARD (vs Adaptive)                                         ");
    println!("=========================================================================================================================");
    println!("EXP197-Contrastive Combined Win Rate: {:.2}% ({}/{})", c_win_pct, combined_c_wins, total_h2h);
    println!("Adaptive-Terminal Win Rate          : {:.2}% ({}/{})", a_win_pct, combined_a_wins, total_h2h);
    ...
    println!("EXP197 Worst 5% Floor               : ${:.1} (vs Adaptive Floor: ${:.1})", stats_c_vs_a_s0.p0_worst_5pct, stats_c_vs_a_s0.p1_worst_5pct);
    println!("EXP197 Worst 1% Floor               : ${:.1} (vs Adaptive Floor: ${:.1})", stats_c_vs_a_s0.p0_worst_1pct, stats_c_vs_a_s0.p1_worst_1pct);
    println!("=========================================================================================================================");

    // Multi-Tier Population Promotion Gate
    println!("\n--- RUNNING MULTI-TIER POPULATION PROMOTION GATE (16,000 MATCHES) ---");
    let tier_seeds: Vec<u64> = (165000..167000).collect(); // 2,000 held-out seeds x 2 seats x 4 tiers
    ...
    println!("=========================================================================================================================");
    println!("                                   EXP197 MULTI-TIER SCORECARD                                                           ");
    println!("=========================================================================================================================");
