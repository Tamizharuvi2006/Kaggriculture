//! EXP204 — 10,000-Match Head-to-Head Tournament & Multi-Tier Population Gate for EXP204 Elite BC Policy.

use fastsim::engine::state::GameState;
use fastsim::engine::step::step_game;
use fastsim::policies::{
    Policy, AdaptiveTerminalPolicy, EXP204EliteBCPolicy, D1Policy, V41Policy, AgroHybridPolicy
};
use rayon::prelude::*;
    ...
    Opp: Policy + 'static,
{
    let s0_res: Vec<(f64, f64)> = seeds.par_iter().map(|&seed| {
        let hero = EXP204EliteBCPolicy::new();
        let opp = opp_factory();
...
fn main() {
    println!("=========================================================================================================================");
    println!("     EXP204 — 10,000-MATCH TOURNAMENT & MULTI-TIER POPULATION GATE (EXP204 ELITE BC POLICY)                               ");
    println!("=========================================================================================================================");

    let h2h_seeds: Vec<u64> = (700000..705000).collect(); // 5,000 fresh seeds x 2 seats = 10,000 matches
    let t0 = Instant::now();

    println!("\n[1/6] EXP204-EliteBC (Seat 0) vs Adaptive-Terminal (Seat 1)...");
    let stats_a_vs_a_s0 = evaluate_matchup(EXP204EliteBCPolicy::new, AdaptiveTerminalPolicy::new, "EXP204-EliteBC", "Adaptive", &h2h_seeds);

    println!("[2/6] Adaptive-Terminal (Seat 0) vs EXP204-EliteBC (Seat 1)...");
    let stats_a_vs_a_s1 = evaluate_matchup(AdaptiveTerminalPolicy::new, EXP204EliteBCPolicy::new, "Adaptive", "EXP204-EliteBC", &h2h_seeds);
    ...
    println!("=========================================================================================================================");
    println!("EXP204-EliteBC Combined Win Rate: {:.2}% ({}/{})", a0_win_pct, combined_a0_wins, total_h2h);
    ...
    println!("EXP204 Mean Reward              : ${:.1} (vs Adaptive Mean: ${:.1})", (stats_a_vs_a_s0.p0_mean + stats_a_vs_a_s1.p1_mean)/2.0, (stats_a_vs_a_s0.p1_mean + stats_a_vs_a_s1.p0_mean)/2.0);
    println!("Net Margin Delta (Hero - Opp)   : {:+.2}", combined_delta);
    println!("EXP204 Worst 5% Floor           : ${:.1} (vs Adaptive Floor: ${:.1})", stats_a_vs_a_s0.p0_worst_5pct, stats_a_vs_a_s0.p1_worst_5pct);
    println!("EXP204 Worst 1% Floor           : ${:.1} (vs Adaptive Floor: ${:.1})", stats_a_vs_a_s0.p0_worst_1pct, stats_a_vs_a_s0.p1_worst_1pct);
    println!("=========================================================================================================================");

    println!("\n--- RUNNING MULTI-TIER POPULATION PROMOTION GATE (16,000 MATCHES) ---");
    let tier_seeds: Vec<u64> = (705000..707000).collect();
    ...
    println!("=========================================================================================================================");
    println!("                                   EXP204 MULTI-TIER SCORECARD                                                           ");
    println!("=========================================================================================================================");
