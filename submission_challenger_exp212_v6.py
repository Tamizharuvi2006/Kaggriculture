"""Kaggriculture Tournament Agent — EXP208 Champion Policy (Clean-Room Production Build).
...
def agent(obs, configuration=None):
    ...
    try:
        ...
        if step is None:
            step = int(obs.get("day", 0) if isinstance(obs, dict) else getattr(obs, "day", 0) or 0) * 24 + int(obs.get("hour", 0) if isinstance(obs, dict) else getattr(obs, "hour", 0) or 0)
        step = min(max(0, int(step)), 719)
        ...
        if step >= 690:
            ...
            if wheat_in_shed > 0: clean_orders.append(["SELL", "WHEAT", wheat_in_shed])
            if clean_orders:
        ...
        # === RESPONDER V6 SELECTIVE OVERLAY ===
        ...
        for r in (opp_farm.get("tiles") or []):
            for t in r:
        ...
        for r in (own_farm.get("tiles") or []):
            for t in r:
                if isinstance(t, dict):
        ...
        # 1. Land Preemption: strictly when opp_quads >= 2/3 and cash reserve is safe
        if len(unlocked) == 1 and 6 <= day <= 7 and opp_quads >= 2 and money >= 950.0:
            if not any(isinstance(m, (list, tuple)) and len(m) >= 1 and m[0] == "BUY_LAND" for m in market_orders):
                market_orders.insert(0, ["BUY_LAND"])

        if len(unlocked) == 2 and 8 <= day <= 11 and opp_quads >= 3 and money >= 1200.0:
            if not any(isinstance(m, (list, tuple)) and len(m) >= 1 and m[0] == "BUY_LAND" for m in market_orders):
                market_orders.insert(0, ["BUY_LAND"])

        # 2. Livestock Matching: when opp_animals >= 5 and money >= 950
        if len(unlocked) >= 2 and 8 <= day <= 15 and opp_animals >= 5:
            if own_cows < 8 and money >= 950.0 and own_animals < opp_animals:
                if not any(isinstance(m, (list, tuple)) and len(m) >= 2 and m[0] == "BUY_ANIMAL" and m[1] == "COW" for m in market_orders):
                    market_orders.append(["BUY_ANIMAL", "COW"])

        # 3. Selective Feed Economics: suppress market feed ONLY if wool is crashed (<$35) AND milk is low (<$50)
        p_milk = float(prices.get("MILK", 80.0) or 80.0)
        if day >= 22 and (p_wool < 35.0 and p_milk < 50.0):
            market_orders = [m for m in market_orders if not (isinstance(m, (list, tuple)) and len(m) >= 2 and m[0] == "BUY_PRODUCT" and m[1] == "WHEAT")]
