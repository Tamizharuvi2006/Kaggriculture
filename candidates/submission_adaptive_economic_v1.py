"""Kaggriculture Adaptive Economic Agent — V1 Clean Production Architecture.
...
def _crop_plan(day):
    ...
    p_straw = float(_LATEST_PRICES.get("STRAWBERRY", 120.0) or 120.0)
    p_wheat = float(_LATEST_PRICES.get("WHEAT", 25.0) or 25.0)
    p_melon = float(_LATEST_PRICES.get("MELON", 200.0) or 200.0)
    
    targets = _blended_targets()
    strawberries = targets["strawberries"]
    
    num_wheat = 12 if p_wheat >= 26.0 else 8
    if p_straw < 35.0:
        strawberries = min(strawberries, 8)
    elif p_straw < 60.0:
        strawberries = min(strawberries, 16)
    ...
    # 1. Allocate on-farm wheat plots for zero-cost feed + high-margin town sales
    for pos in candidates[:num_wheat]:
        plan[pos] = "WHEAT"
        
    # 2. Allocate strawberries up to price-adjusted cap
    rem = candidates[num_wheat:]
    for pos in rem[:max(0, int(strawberries))]:
        plan[pos] = "STRAWBERRY"
        
    # 3. Allocate any remaining plots to Melon (if early) or Carrot
    rem_2 = rem[max(0, int(strawberries)):]
    for pos in rem_2:
        plan[pos] = "MELON" if day <= 16 else "CARROT"
...
def _market_orders(obs):
    ...
    fertilizer_sale = int(shed.get("FERTILIZER", 0))
    ...
    critical_target = min(target_hires, 2 if day == 0 else 2 if day <= 4 else 4)
    ...
    liquidity_floor = 0 if day >= 15 else (250 if day <= 5 else 100)
