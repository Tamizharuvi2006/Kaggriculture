...
def _v18_closed_loop_action(obs, step):
    ...
    market_orders = [list(order) for order in (market_action.get("market") or [])]
    ...
    # Competitive Hybrid V12 Dynamic Meta-Weight Architecture (+ L+++ Safety Net)
    ...
    # Opponent State & Trajectory Slope Predictor
    ...
    # 🚀 Dynamic Meta-Weight Controller (Shifts w_opt, w_exp, w_adv dynamically based on regime)
    if wealth_delta < -10000.0:
        # Growth Required Mode: Shift weight to Optimistic compounding trajectory
        w_opt, w_exp, w_adv = 0.5, 0.3, 0.2
    elif wealth_delta > 20000.0:
        # Lead Protection Mode: Shift weight to Adversarial safety trajectory
        w_opt, w_exp, w_adv = 0.1, 0.4, 0.5
    else:
        # Balanced Duel Mode: Normal EV expectation
        w_opt, w_exp, w_adv = 0.2, 0.6, 0.2

    # Distributional Ensemble Trajectories
    m_inv = shed.get("MILK", 0)
    ev_optimistic = own_money + (cows * 220.0 * 0.5 + 60.0) * (turns_remaining / 36.0)
    ev_expected = own_money + (cows * milk_price * 0.5 + 30.0) * (turns_remaining / 36.0)
    ev_adversarial = own_money + (cows * 140.0 * 0.5 + 15.0) * (turns_remaining / 36.0)

    # Meta-Weighted Trajectory Utility Score
    meta_trajectory_score = w_exp * ev_expected + w_adv * ev_adversarial + w_opt * ev_optimistic

    # $200K+ Peak Ceiling Breaker Trigger Condition
    is_meta_200k_ceiling = (meta_trajectory_score >= 105000.0 and milk_price >= 180.0 and turns_remaining >= 180)

    # Competitive Regime Classifier
    comp_regime = "CLOSE"
    if wealth_delta >= 40000.0 or is_meta_200k_ceiling:
        comp_regime = "MASSIVE_LEAD"       # 🚀 META-WEIGHT $200K CEILING BREAKER MODE
    elif wealth_delta >= 15000.0:
        comp_regime = "LEADING"            # WEALTH ACCELERATION MODE
    elif wealth_delta >= -10000.0:
        comp_regime = "CLOSE"              # MARGIN OPTIMIZATION
    ...
    # Action Triggers
    is_max_compounding = (comp_regime in ("MASSIVE_LEAD", "LEADING") and milk_price >= 180.0 and turns_remaining >= 180)
    is_high_wealth_trajectory = (is_meta_200k_ceiling or own_money >= 500.0)
    ...
    if step_val >= 710:
        ...
        regime = "WHEAT_GLUT"
    elif is_max_compounding:
        regime = "MAX_COMPOUNDING"           # 🚀 Pursues $200k ceiling when holding lead
    elif is_recovery_mode or is_high_wealth_trajectory:
        regime = "RECOVERY_OPPORTUNITY"
    elif milk_price >= 200.0:
        regime = "MILK_PREMIUM"
    elif step_val <= 200 and own_money < 5000:
        regime = "LOW_LIQUIDITY"

    # Confidence Gateway Evaluation
    confidence = "LOW"
    if regime in ("WHEAT_GLUT", "ENDGAME", "MILK_PREMIUM", "RECOVERY_OPPORTUNITY", "MAX_COMPOUNDING"):
        confidence = "HIGH"
    elif regime == "LOW_LIQUIDITY" and own_money >= 500:
        confidence = "MEDIUM"

    # Action Selection: HIGH CONFIDENCE -> V12 Meta-Weight Controller Override; LOW/MEDIUM -> Candidate L+++ Guardian Fallback
    if confidence == "HIGH" and regime == "MAX_COMPOUNDING":
        # 🚀 META-WEIGHT CEILING BREAKER: Maximize dynamic ensemble utility
        if m_inv >= 2:
            if not any(isinstance(o, list) and len(o) > 1 and o[0] == "SELL" and o[1] == "MILK" for o in market_orders):
                market_orders.insert(0, ["SELL", "MILK", m_inv])
    elif confidence == "HIGH" and regime == "RECOVERY_OPPORTUNITY":
        if m_inv >= 4:
            if not any(isinstance(o, list) and len(o) > 1 and o[0] == "SELL" and o[1] == "MILK" for o in market_orders):
                market_orders.insert(0, ["SELL", "MILK", m_inv])
    elif confidence == "HIGH" and regime == "WHEAT_GLUT":
        w_inv = shed.get("WHEAT", 0)
        if w_inv > 0 and not any(isinstance(o, list) and len(o) > 1 and o[0] == "SELL" and o[1] == "WHEAT" for o in market_orders):
            market_orders.append(["SELL", "WHEAT", w_inv])
    elif confidence == "HIGH" and regime == "ENDGAME":
        flush_orders = []
        for crop_item in ["MILK", "WOOL", "STRAWBERRY", "MELON", "WHEAT"]:
        ...
        # Candidate L+++ Guardian Baseline Rules
        if step_val >= 718:
            flush_orders = []
