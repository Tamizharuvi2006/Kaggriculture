"""Fast Synchronous Paired A/B Benchmark with Explicit Flushing: Candidate L+ vs Frozen V4.1 Master.

Evaluates Candidate L+ vs Frozen V4.1 across 20 Paired Matches on Seeds 3000-3004 (5 paired matches per archetype):
...
def main():
    print("=" * 95, flush=True)
    print(" CONTROLLED PAIRED A/B BENCHMARK: CANDIDATE L+ VS FROZEN V4.1 MASTER", flush=True)
    print("=" * 95, flush=True)
    print(" Launching 20 Paired Matches (60 Total Matches, Seeds 3000-3004)...", flush=True)

    v41_mod = _load_mod("v41_baseline", V18_PATH)
    ...
    lplus_mod = _load_mod("lplus_candidate", LPLUS_PATH)

    turtle_mod = _load_mod("turtle_opp", V18_PATH)
    turtle_mod.configure_strategy({"use_fixed_schedule": False, "v13_market_adaptation": True, "opening_melons": 10, "cows": 8})

    rusher_mod = _load_mod("rusher_opp", V18_PATH)
    rusher_mod.configure_strategy({"use_fixed_schedule": False, "cows": 12, "opening_melons": 15})

    manipulator_mod = _load_mod("manipulator_opp", V18_PATH)
    manipulator_mod.configure_strategy({"use_fixed_schedule": False, "v13_market_adaptation": False, "opening_melons": 15, "cows": 8})

    expansionist_mod = _load_mod("expansionist_opp", V18_PATH)
    expansionist_mod.configure_strategy({"use_fixed_schedule": False, "land_ne_day": 4, "strawberries": 50, "opening_melons": 15, "cows": 8})

    opp_map = {
        "Capital_Turtle": turtle_mod.agent,
        "Cattle_Rusher": rusher_mod.agent,
        "Market_Manipulator": manipulator_mod.agent,
        "Crop_Expansionist": expansionist_mod.agent,
    }

    archetypes = ["Capital_Turtle", "Cattle_Rusher", "Market_Manipulator", "Crop_Expansionist"]
    seeds = range(3000, 3005)  # 5 seeds per archetype = 20 paired matches

    results = []

    for arch_name in archetypes:
        opp_agent = opp_map[arch_name]
        for seed in seeds:
            # Match 1: Candidate L+ vs Opponent O
            env1 = kaggle_environments.make("kaggriculture", configuration={"episodeSteps": 720, "seed": seed})
            state1 = env1.run([lplus_mod.agent, opp_agent])
            ...
            state3 = env3.run([lplus_mod.agent, v41_mod.agent])
            ...
            print(f" Completed Seed {seed} vs {arch_name:20} | L+: ${lplus_w:9.2f} | V4.1: ${v41_w:9.2f} | Delta: +${delta:9.2f}", flush=True)
    ...
    print("\n" + "=" * 95, flush=True)
    print(" PAIRED A/B RESULTS BY OPPONENT ARCHETYPE (5 SEEDS EACH)", flush=True)
    print("=" * 95, flush=True)
    print(f"{'Archetype':<20} | {'Direct Win %':<12} | {'L+ Avg ($)':<12} | {'V4.1 Avg ($)':<12} | {'Mean Delta ($)':<15} | {'p-value':<10} | {'Status'}", flush=True)
    print("-" * 95, flush=True)
    ...
    for arch, s in archetype_summary.items():
        ...
        print(f"{arch:<20} | {direct_wr:5.1f}%       | ${mean_lplus:10.2f} | ${mean_v41:10.2f} | +${mean_delta:12.2f} | {p_val:8.4f}   | {status}", flush=True)
    ...
    print("=" * 95, flush=True)
    print(f" OVERALL COMBINED A/B BENCHMARK (20 PAIRED MATCHES / 60 TOTAL MATCHES):", flush=True)
    print(f" - Candidate L+ Direct Win Rate vs V4.1: {overall_direct_wr:.1f}% ({all_direct_lplus_wins}/{total_n} Wins)", flush=True)
    print(f" - Overall Mean Paired Wealth Delta: +${overall_mean_delta:.2f}", flush=True)
    print(f" - Overall Median Paired Wealth Delta: +${median_delta:.2f}", flush=True)
    print(f" - Minimum Delta (Worst Case): ${min_delta:.2f}", flush=True)
    print(f" - Maximum Delta (Best Case): +${max_delta:.2f}", flush=True)
    print(f" - Paired t-Test p-value: {overall_p:.6e}", flush=True)
    print("=" * 95, flush=True)
    ...
    print(f"\nSaved complete raw A/B dataset to {output_path}", flush=True)
