...
def _v18_closed_loop_action(obs, step):
    ...
    # Competitive Hybrid V8 Counterfactual Trajectory Search Architecture (+ L+++ Safety Net)
    ...
    # Counterfactual Trajectory Search Engine (Simulating Action A, B, C Terminal Wealth at Turn 720)
    # Action A: Sell Milk Now -> Immediate cash
    # Action B: Reinvest in Cow Stock -> Long-term Milk daily compounding
    # Action C: Build Pasture #2 -> Double livestock capacity threshold
    daily_cow_rev = cows * milk_price * 0.5
    projected_terminal_wealth_sell = own_money + shed.get("MILK", 0) * milk_price
    projected_terminal_wealth_reinvest = own_money + (daily_cow_rev + 15.0) * (turns_remaining / 36.0)
    
    # Counterfactual Action Selection: Pick highest risk-adjusted terminal wealth projection
    best_action_future = "REINVEST" if projected_terminal_wealth_reinvest > projected_terminal_wealth_sell else "SELL"

    # $200K Ceiling Bottleneck Detector: Unlocks cash/pasture/cow bottlenecks when projected ceiling < $200K
    is_bottleneck_unlocked = (pasture_count < 2 and own_money >= 500.0 and turns_remaining >= 250)

    # Competitive Regime Classifier
    comp_regime = "CLOSE"
    if wealth_delta >= 40000.0:
        comp_regime = "MASSIVE_LEAD"       # 🚀 $200K CEILING BOTTLENECK UNLOCKER
    ...
    # Action Triggers
    is_max_compounding = (comp_regime in ("MASSIVE_LEAD", "LEADING") and milk_price >= 180.0 and turns_remaining >= 250)
    is_high_wealth_trajectory = (is_bottleneck_unlocked or best_action_future == "REINVEST")
    ...
    # Action Selection: HIGH CONFIDENCE -> V8 Counterfactual Search Override; LOW/MEDIUM -> Candidate L+++ Guardian Fallback
    if confidence == "HIGH" and regime == "MAX_COMPOUNDING":
        # 🚀 COUNTERFACTUAL CEILING UNLOCKER: Maximize projected Turn-720 terminal wealth
