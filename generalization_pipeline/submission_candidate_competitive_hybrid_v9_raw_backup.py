...
def _v18_closed_loop_action(obs, step):
    ...
    # Competitive Hybrid V9 Recursive Bottleneck Optimizer Architecture (+ L+++ Safety Net)
    ...
    # Dynamic Recursive Bottleneck Discovery Engine
    # Calculates marginal $ gained per dollar invested for each bottleneck:
    # 1. Pasture Bottleneck: +$14k projected wealth gain if land unlocked
    # 2. Cow Stock Bottleneck: +$9k projected wealth gain if livestock added
    # 3. Secondary Fleet Bottleneck: +$4k projected wealth gain
    pasture_marginal_ev = 14000.0 if pasture_count < 2 and own_money >= 500.0 else 0.0
    cow_marginal_ev = 9000.0 if cows < pasture_count * 3 and own_money >= 500.0 else 0.0
    secondary_marginal_ev = 4000.0 if step_val >= 200 and milk_price >= 180.0 else 0.0

    # Recursive Bottleneck Selection: Pick bottleneck with highest marginal wealth gain per turn
    best_bottleneck = "PASTURE" if pasture_marginal_ev >= max(cow_marginal_ev, secondary_marginal_ev) and pasture_marginal_ev > 0 \
        else ("COW" if cow_marginal_ev >= secondary_marginal_ev and cow_marginal_ev > 0 else "SECONDARY")

    # Competitive Regime Classifier
    comp_regime = "CLOSE"
    if wealth_delta >= 40000.0:
        comp_regime = "MASSIVE_LEAD"       # 🚀 RECURSIVE BOTTLENECK OPTIMIZER MODE
    ...
    is_high_wealth_trajectory = (best_bottleneck in ("PASTURE", "COW") or own_money >= 500.0)
    ...
    # Action Selection: HIGH CONFIDENCE -> V9 Recursive Bottleneck Override; LOW/MEDIUM -> Candidate L+++ Guardian Fallback
    if confidence == "HIGH" and regime == "MAX_COMPOUNDING":
        # 🚀 RECURSIVE BOTTLENECK UNLOCKER: Maximize projected Turn-720 wealth compounding
