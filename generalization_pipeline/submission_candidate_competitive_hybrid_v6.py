...
def _v18_closed_loop_action(obs, step):
    ...
    # Competitive Hybrid V6 Trajectory-Aware Risk-Adjusted EV Architecture (+ L+++ Safety Net)
    ...
    # Risk-Adjusted Projected Turn-720 Wealth EV Calculation
    # Projected Final Wealth = Money + Expected Production Revenue + Expected Reinvestment Return - Costs
    expected_daily_milk_rev = cows * milk_price * 0.5
    projected_turn720_rev = expected_daily_milk_rev * (turns_remaining / 36.0)
    projected_final_wealth = own_money + projected_turn720_rev

    # Competitive Regime Classifier & Dynamic Risk-Adjusted EV Gates
    comp_regime = "CLOSE"
    if wealth_delta >= 40000.0:
        comp_regime = "MASSIVE_LEAD"       # 🚀 HIGH EV + CEILING SEEKING MODE
    elif wealth_delta >= 15000.0:
        comp_regime = "LEADING"            # HIGH EV + LOW RISK COMPOUNDING
    elif wealth_delta >= -10000.0:
        comp_regime = "CLOSE"              # HIGH EV MARGIN OPTIMIZATION
    elif wealth_delta >= -35000.0:
        comp_regime = "TRAILING"           # RECOVERY COMPOUNDING
    else:
        comp_regime = "SEVERELY_TRAILING"   # 🚨 HIGH EV + HIGH RISK RECOVERY MODE ($40k vs $120k)

    # Capital Velocity & Risk-Adjusted EV Action Triggers
    is_max_compounding = (comp_regime == "MASSIVE_LEAD" and milk_price >= 180.0 and turns_remaining >= 250)
    ...
    if step_val >= 710:
        ...
        regime = "MAX_COMPOUNDING"           # 🚀 Pursues $200k ceiling when holding massive lead
    elif is_wealth_acceleration:
        regime = "HIGH_WEALTH_ACCELERATOR"  # Pursues $150k-$200k ceiling when leading
    ...
    confidence = "LOW"
    if regime in ("WHEAT_GLUT", "ENDGAME", "MILK_PREMIUM", "RECOVERY_OPPORTUNITY", "HIGH_WEALTH_ACCELERATOR", "MAX_COMPOUNDING"):
        confidence = "HIGH"
    elif regime == "LOW_LIQUIDITY" and own_money >= 500:
        confidence = "MEDIUM"

    # Action Selection: HIGH CONFIDENCE -> Risk-Adjusted EV Override; LOW/MEDIUM -> Candidate L+++ Guardian Fallback
    if confidence == "HIGH" and regime == "MAX_COMPOUNDING":
        # 🚀 MAX_COMPOUNDING Mode: Risk-Adjusted EV Action Selection
    ...
    elif confidence == "HIGH" and regime == "HIGH_WEALTH_ACCELERATOR":
        m_inv = shed.get("MILK", 0)
        if m_inv >= 2:
            if not any(isinstance(o, list) and len(o) > 1 and o[0] == "SELL" and o[1] == "MILK" for o in market_orders):
                market_orders.insert(0, ["SELL", "MILK", m_inv])
