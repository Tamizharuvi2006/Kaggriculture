...
def _v18_closed_loop_action(obs, step):
    ...
    # Competitive Hybrid V13 Competitive Game-Theoretic MPC Architecture (+ L+++ Safety Net)
    ...
    # 🚀 Competitive Game-Theoretic MPC Engine (Simulating our_trajectory - opponent_trajectory)
    m_inv = shed.get("MILK", 0)
    our_projected_compounding = own_money + (cows * milk_price * 0.5 + 35.0) * (turns_remaining / 36.0)
    opp_projected_compounding = opp_money + 1500.0 * (turns_remaining / 36.0)
    relative_margin_ev = our_projected_compounding - opp_projected_compounding

    # Win-Probability + Margin Utility Model
    # Utility = Wealth + 1.5 * Margin + Win_Prob * Compounding
    win_probability_weight = 1.2 if relative_margin_ev > 0 else 0.4
    game_theoretic_utility = our_projected_compounding + 1.5 * max(0.0, relative_margin_ev) * win_probability_weight

    # $200K+ Peak Ceiling Breaker Trigger Condition
    is_v13_200k_ceiling = (game_theoretic_utility >= 110000.0 and milk_price >= 180.0 and turns_remaining >= 180)

    # Competitive Regime Classifier
    comp_regime = "CLOSE"
    if wealth_delta >= 40000.0 or is_v13_200k_ceiling:
        comp_regime = "MASSIVE_LEAD"       # 🚀 GAME-THEORETIC $200K CEILING BREAKER MODE
    ...
    is_high_wealth_trajectory = (is_v13_200k_ceiling or own_money >= 500.0)
    ...
    # Action Selection: HIGH CONFIDENCE -> V13 Game-Theoretic MPC Override; LOW/MEDIUM -> Candidate L+++ Guardian Fallback
    if confidence == "HIGH" and regime == "MAX_COMPOUNDING":
        # 🚀 GAME-THEORETIC CEILING BREAKER: Maximize relative margin utility
        if m_inv >= 2:
