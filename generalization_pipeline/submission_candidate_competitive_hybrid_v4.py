...
def _v18_closed_loop_action(obs, step):
    ...
    confidence = "LOW"
    if regime in ("WHEAT_GLUT", "ENDGAME", "MILK_PREMIUM", "RECOVERY_OPPORTUNITY", "HIGH_WEALTH_ACCELERATOR"):
        confidence = "HIGH"
    elif regime == "LOW_LIQUIDITY" and own_money >= 500:
        confidence = "MEDIUM"

    # Action Selection: HIGH CONFIDENCE -> V4 Multi-Module Override; LOW/MEDIUM -> Candidate L+++ Guardian Fallback
    if confidence == "HIGH" and regime == "HIGH_WEALTH_ACCELERATOR":
        # 🚀 High-Wealth Accelerator: Once ahead, double down on multi-pasture compounding to reach $150k+
        ...
        # 🚨 RECOVERY / Trajectory Engine: Maximize EV action selection & multi-crop compounding
        m_inv = shed.get("MILK", 0)
        if m_inv >= 4:
            if not any(isinstance(o, list) and len(o) > 1 and o[0] == "SELL" and o[1] == "MILK" for o in market_orders):
                market_orders.insert(0, ["SELL", "MILK", m_inv])
