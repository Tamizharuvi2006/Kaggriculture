...
def _v18_closed_loop_action(obs, step):
    ...
    # Competitive Hybrid V7 Wealth Ceiling Breaker Architecture (Slope Predictor + L+++ Safety Net)
    ...
    # Marginal ROI Engine per Remaining Turn
    # Payback time < remaining_turns AND ROI_remaining > current_best_alternative
    payback_turns = 100.0 if cows == 0 else (500.0 / (cows * milk_price * 0.5 + 1.0))
    has_high_roi_reinvestment = (payback_turns < turns_remaining)

    # Opponent Trajectory Slope Predictor
    # Is our growth rate threatened by opponent growth slope?
    our_growth_slope = cows * milk_price * 0.5
    opp_growth_slope = opp_money / max(1.0, float(step_val))
    is_threatened_lead = (wealth_delta > 0 and opp_growth_slope > our_growth_slope)

    # Competitive Regime Classifier
    comp_regime = "CLOSE"
    if wealth_delta >= 40000.0 or (wealth_delta >= 15000.0 and not is_threatened_lead):
        comp_regime = "MASSIVE_LEAD"       # 🚀 CEILING MODE (MAX COMPOUNDING)
    elif is_threatened_lead:
        comp_regime = "THREATENED_LEAD"    # ⚠️ THREATENED LEAD (COMPOUND HARDER)
    ...
    # Capital Velocity & Ceiling Breaker Action Triggers
    is_max_compounding = (comp_regime in ("MASSIVE_LEAD", "THREATENED_LEAD") and milk_price >= 180.0 and turns_remaining >= 250)
    is_high_wealth_trajectory = (pasture_count < 2 and own_money >= 500.0 and turns_remaining >= 350 and has_high_roi_reinvestment)
    ...
    # Action Selection: HIGH CONFIDENCE -> V7 Ceiling Breaker Override; LOW/MEDIUM -> Candidate L+++ Guardian Fallback
    if confidence == "HIGH" and regime == "MAX_COMPOUNDING":
        # 🚀 CEILING MODE: Maximize projected Turn-720 wealth compounding
