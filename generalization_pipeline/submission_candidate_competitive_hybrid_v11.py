...
def _v18_closed_loop_action(obs, step):
    ...
    # Competitive Hybrid V11 Distributional Ensemble MPC Architecture (+ L+++ Safety Net)
    ...
    # 🚀 Distributional Ensemble Trajectory Simulator (Optimistic, Expected, Adversarial Ensembles)
    # Scenario 1 (Optimistic): High Milk price ($220) & high growth rate -> EV_opt
    # Scenario 2 (Expected): Normal EV compounding -> EV_exp
    # Scenario 3 (Adversarial): Opponent market pressure & lower price -> EV_adv
    m_inv = shed.get("MILK", 0)
    ev_optimistic = own_money + (cows * 220.0 * 0.5 + 50.0) * (turns_remaining / 36.0)
    ev_expected = own_money + (cows * milk_price * 0.5 + 25.0) * (turns_remaining / 36.0)
    ev_adversarial = own_money + (cows * 140.0 * 0.5 + 10.0) * (turns_remaining / 36.0)

    # Robust Trajectory Scoring Function
    # Score = 0.5 * EV_expected + 0.3 * EV_adversarial + 0.2 * EV_optimistic
    robust_trajectory_score = 0.5 * ev_expected + 0.3 * ev_adversarial + 0.2 * ev_optimistic

    # $200K+ Robust Ceiling Breaker Trigger Condition
    is_robust_200k_ceiling = (robust_trajectory_score >= 105000.0 and milk_price >= 180.0 and turns_remaining >= 200)

    # Competitive Regime Classifier
    comp_regime = "CLOSE"
    if wealth_delta >= 40000.0 or is_robust_200k_ceiling:
        comp_regime = "MASSIVE_LEAD"       # 🚀 $200K ENSEMBLE CEILING BREAKER MODE
    ...
    is_high_wealth_trajectory = (is_robust_200k_ceiling or own_money >= 500.0)
    ...
    # Action Selection: HIGH CONFIDENCE -> V11 Distributional Ensemble Override; LOW/MEDIUM -> Candidate L+++ Guardian Fallback
    if confidence == "HIGH" and regime == "MAX_COMPOUNDING":
        # 🚀 $200K ENSEMBLE CEILING BREAKER: Maximize robust trajectory terminal wealth
        if m_inv >= 2:
