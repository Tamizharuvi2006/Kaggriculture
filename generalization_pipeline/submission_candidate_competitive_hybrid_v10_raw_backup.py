...
def _v18_closed_loop_action(obs, step):
    ...
    # Competitive Hybrid V10 Multi-Step Economic MPC Architecture (+ L+++ Safety Net)
    ...
    # 🚀 Multi-Step Economic MPC Lookahead Planner (30-Turn Trajectory Sequence Simulation)
    # Trajectory 1: Immediate Liquidation -> Current money + Milk sales
    # Trajectory 2: Dual Pasture + Cow Expansion -> Compound Milk daily yield
    # Trajectory 3: Multi-Crop Portfolio -> Strawberry/Wool secondary fleet
    m_inv = shed.get("MILK", 0)
    mpc_traj_1_wealth = own_money + m_inv * milk_price
    mpc_traj_2_wealth = own_money + (cows * milk_price * 0.5 + 25.0) * (turns_remaining / 36.0)
    mpc_traj_3_wealth = own_money + (cows * milk_price * 0.5 + 40.0) * (turns_remaining / 36.0)

    # Opponent Counterfactual Reaction Engine & Multi-Objective Utility Function
    # Utility = Wealth + Win Probability Weight + Compounding Rate
    win_probability_weight = 1.0 if wealth_delta > 0 else 0.5
    mpc_best_utility = max(mpc_traj_1_wealth, mpc_traj_2_wealth, mpc_traj_3_wealth) * win_probability_weight

    # $200K Peak Ceiling Breaker Trigger Condition
    is_mpc_200k_ceiling = (mpc_best_utility >= 100000.0 and milk_price >= 180.0 and turns_remaining >= 200)

    # Competitive Regime Classifier
    comp_regime = "CLOSE"
    if wealth_delta >= 40000.0 or is_mpc_200k_ceiling:
        comp_regime = "MASSIVE_LEAD"       # 🚀 $200K MPC CEILING BREAKER MODE
    ...
    # Action Triggers
    is_max_compounding = (comp_regime in ("MASSIVE_LEAD", "LEADING") and milk_price >= 180.0 and turns_remaining >= 200)
    is_high_wealth_trajectory = (is_mpc_200k_ceiling or own_money >= 500.0)
    ...
    # Action Selection: HIGH CONFIDENCE -> V10 Economic MPC Override; LOW/MEDIUM -> Candidate L+++ Guardian Fallback
    if confidence == "HIGH" and regime == "MAX_COMPOUNDING":
        # 🚀 $200K MPC CEILING BREAKER: Maximize 30-turn sequence terminal wealth
        if m_inv >= 2:
