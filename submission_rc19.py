...
def _crop_plan(day):
    ...
    # Two melon cycles on the 9 opening sites (v18's measured 117 vs our 65
    # cleared melons). After Day 22 those tiles rotate to wheat — too late
    # for a strawberry first-yield. Strawberries stay on every other cash tile
    # while a first yield still fits; spent plants rotate to wheat.
    plan = {}
    if day < 22 and crop_scores.get("MELON", -999.0) > 0:
        plan = {pos: crop for pos, crop in OPENING_CROP_PLAN.items() if crop == "MELON"}
    ...
    # Late season: a strawberry planted after last_plant never yields. Rotate
    # those tiles to wheat (2-day cycle) — this is the v18 ceiling engine
    # (674 cleared wheat vs RC18's 22 on seed 100), not a second cow.
    last_straw = int(STRATEGY.get("strawberry_last_plant", 19))
    if day > last_straw:
        plan = {pos: ("WHEAT" if crop == "STRAWBERRY" else crop) for pos, crop in plan.items()}
    return plan
...
def _crop_is_spent(tile, day):
    """True when an ongoing plant has finished its production events.

    Engine sets max_lifespan_step on the 4th strawberry tick (age 16). The
    tile then decays to a weed; digging immediately frees it for gen-2
    strawberry (if last_plant still allows) or wheat rotation.
    """
    crop = tile.get("crop")
    spec = CROPS.get(crop)
    if not spec or tile.get("kind") != "PLANT":
        return False
    if int(tile.get("yield_units", 0)) > 0:
        return False
    age = day - int(tile.get("planted_day", day))
    if spec["ongoing"]:
        if int(tile.get("max_lifespan_step", -1)) >= 0:
            return True
        if crop == "STRAWBERRY" and age >= 16:
            return True
        if crop == "TOMATO" and age >= 12:
            return True
        return False
...
def _build_tasks(obs, positions, inventories):
        ...
        for x, tile in enumerate(row):
            ...
            if tile.get("animal") in ANIMALS:
                ...
                if day < 29 and not tile.get("fed_today", False):
                    # Always prio 0: rotation DIG/plant must never starve the herd.
                    tasks.append(_task(0, (x, y), ["FEED"], "WHEAT", "feed", 800.0))
                if int(tile.get("yield_units", 0)) > 0:
    ...
    for (x, y), desired in crop_plan.items():
        ...
        if isinstance(tile, dict) and tile.get("kind") == "PLANT":
            ...
            if _crop_is_ripe(tile, day, hour):
                ...
                tasks.append(_task(1, (x, y), ["HARVEST"], None, "harvest", ev))
            elif _crop_is_spent(tile, day) or (
                desired != crop
                and int(tile.get("yield_units", 0)) <= 0
                and not spec.get("ongoing")
            ):
                # Rotation beats routine watering: spent strawberries and
                # finished one-shots must free the tile while wheat last_plant
                # (day 24) is still open.
                tasks.append(_task(1, (x, y), ["DIG"], None, "dig", 220.0))
    ...
    for pos, crop in crop_plan.items():
        ...
        if isinstance(tile, dict) and tile.get("kind") == "WEED":
            # Always clear weeds so spent strawberry tiles can rotate. Planting
            # is still gated by last_plant below.
            tasks.append(_task(1, pos, ["DIG"], None, "dig", 200.0))
            ...
            if crop == "STRAWBERRY":
                ...
                ev = 160.0 if day > 19 else 50.0
            else:
                ev = 30.0
            tasks.append(_task(1 if (crop == "WHEAT" and day > 19) else 2, pos, ["PLANT", crop], None, "plant", ev))
            remaining[crop] -= 1
...
def _quadrant_crop_deficits(obs):
    ...
    for pos, crop in _crop_plan(day).items():
        ...
        if tile is None or (isinstance(tile, dict) and tile.get("kind") == "WEED"):
            deficits[crop] += 1
        elif isinstance(tile, dict) and tile.get("kind") == "PLANT" and _crop_is_spent(tile, day):
            deficits[crop] += 1
...
def _market_orders(obs):
    ...
    # Keep one feeding plus a small buffer (v18). Holding animal*2+2 left
    # ~26 wheat idle in the shed while v18 cleared 674 wheat units.
    wheat_feed_buffer = 0 if day >= 29 else (animal_count + 3)
