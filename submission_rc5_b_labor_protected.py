...
def _build_tasks(obs, positions, inventories):
    ...
    for pos, animal in animal_plan.items():
        ...
        if tile is None:
            # RC5-B Labor Protection: Defer speculative empty pasture building to afternoon (Hour >= 14)
            # during opening days (Days 0-3) unless an animal is physically waiting.
            if waiting or day >= 4 or hour >= 14:
