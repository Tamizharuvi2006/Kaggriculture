...
def _build_tasks(obs, positions, inventories):
        ...
        for x, tile in enumerate(row):
            ...
            if tile.get("animal") in ANIMALS:
                ...
                if not tile.get("cared_today", False) and day < 29:
                    tasks.append(_task(2, (x, y), ["CARE"], None, "care"))
    ...
    for pos, animal in animal_plan.items():
        ...
        if tile is None:
            tasks.append(_task(prio, pos, ["BUILD_PASTURE"], None, "build"))
            ...
            tasks.append(_task(prio, pos, ["DIG"], None, "dig"))
    ...
    for pos, crop in crop_plan.items():
        ...
        if isinstance(tile, dict) and tile.get("kind") == "WEED":
            if day <= _last_plant(crop):
                tasks.append(_task(3, pos, ["DIG"], None, "dig"))
        elif tile is None and day <= _last_plant(crop) and remaining[crop] > 0:
            tasks.append(_task(2, pos, ["PLANT", crop], None, "plant"))
            remaining[crop] -= 1
...
def _assign_worker_zones(positions, tasks, unlocked):
    """Dynamically assign each worker to the quadrant that maximizes productivity and minimizes travel."""
    valid_quads = ["NW"]
    if "NE" in unlocked: valid_quads.append("NE")
    if "SW" in unlocked: valid_quads.append("SW")
    ...
    demand = {q: 0.1 for q in valid_quads}
    for t in tasks:
        q = _quadrant(t[1])
        if q in demand:
            weight = 3.0 if t[0] <= 1 else 1.0
            demand[q] += weight
            
    total_demand = sum(demand.values())
    
    # Target worker quotas per quadrant proportional to demand
    quotas = {q: int(round(n_workers * (demand[q] / total_demand))) for q in valid_quads}
    while sum(quotas.values()) > n_workers:
        max_q = max(valid_quads, key=lambda q: (quotas[q], quotas[q] / max(0.1, demand[q])))
        quotas[max_q] = max(0, quotas[max_q] - 1)
    while sum(quotas.values()) < n_workers:
        min_q = max(valid_quads, key=lambda q: demand[q] / max(0.1, quotas[q]))
        quotas[min_q] += 1
        
    worker_zones = {}
    remaining_workers = set(range(n_workers))
    ...
    # 1. Workers already inside a quadrant stay in that quadrant if quota allows
    for i in list(remaining_workers):
        pos = positions[i]
        cur_q = _quadrant(pos)
        if cur_q in remaining_quotas and remaining_quotas[cur_q] > 0:
            worker_zones[i] = cur_q
            remaining_quotas[cur_q] -= 1
            remaining_workers.remove(i)
            
    # 2. Remaining workers assigned to closest quadrant center with open quota
    for i in list(remaining_workers):
        pos = positions[i]
        open_quads = [q for q in valid_quads if remaining_quotas[q] > 0]
        if not open_quads:
            worker_zones[i] = "NW"
            continue
        best_q = min(open_quads, key=lambda q: _distance(pos, quad_centers[q]))
        worker_zones[i] = best_q
        remaining_quotas[best_q] -= 1
        remaining_workers.remove(i)
...
def _assign_actions(obs):
    ...
    unlocked = set(_get(farm, "unlocked_quadrants", ["NW"]) or ["NW"])
    worker_zones = _assign_worker_zones(positions, tasks, unlocked)
            ...
            for unit in free:
                ...
                for j, task in enumerate(pending):
                    ...
                    if carried_animal and task[4] != "place":
                        dist += 100
                    if task[4] not in ("pickup_wheat", "pickup_fertilizer", "pickup_animal"):
                        if _quadrant(task[1]) != worker_zones.get(unit, "NW"):
                            dist += 25
